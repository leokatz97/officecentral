...
| Goal | Prompt |
|---|---|
| Start the next task | `What's the next ⬜ row in Wave A? Read docs/plan/bbi-build-state.md, propose how to start, wait for confirm.` |
| Build a specific row | `Work on PB-12 — read docs/plan/bbi-build-state.md row PB-12, propose the implementation, then build it. When done, mark the row ✅ with the commit SHA.` |
| Build a Phase 2 page | `/bbi-build-page seating — see BUILD-STATE row P2-2 and INTERLINKING row /collections/seating. Pre-flight: confirm pwd is worktree and BBI_PUSH_ROOT is set.` |
...
| Drift-check a page | `Compare the live /pages/quote on dev theme 186373570873 to theme/sections/ds-lp-quote.liquid in this worktree. Report any drift (suggests a stale push happened).` |
| Update status | `Mark BUILD-STATE row P2-2 as ✅, evidence = commit <sha>. Then re-run audit point 12 to confirm no drift.` |
...
|---|---|---|---|---|---|
| 1 | **PB-12** | **🔴 Fix `bbi-push-landing.py` root detection + extend to layout/snippets** | ✅ | commit `5888659` `scripts/bbi-push-landing.py` | **Highest-leverage fix in the entire plan.** Two parts: (a) Auto-detect repo root — if `pwd` is inside a `.claude/worktrees/*/`, use that as `BBI_PUSH_ROOT` instead of the default main repo path. Or warn loudly when `pwd` differs from the resolved root. (b) Extend `--layout` and `--snippets` flags so theme.liquid + shared bbi-* snippets push without direct API calls. Without (a), every `--slug` push from a worktree silently overwrites Shopify with stale main-repo files (root cause of `9c8b7db` re-sync). |
| 2 | **SKILL-1** | **Harden `/bbi-build-page` skill** | ⬜ | `.claude/skills/bbi-build-page/SKILL.md` | **Blocks Phase 2.** Add Pre-Step 0 (gate + template-JSON identification + verify `pwd` is worktree + verify `BBI_PUSH_ROOT` is set), Step 9 (gate-edit + direct push of `theme.liquid`), Step 10 (template JSON populate — logo + every image_picker setting), Step 11 (post-deploy verify gate — all DOM checks from interlinking-map.md), Step 12 (mark done only on green). Pin version. Bake all 4 lessons-learned into the prevention checklist. Reads `docs/plan/bbi-build-state.md` and `docs/plan/bbi-interlinking-map.md` as Step 0 context. |
| 3 | NAV-1 | Lock canonical nav spec | ⬜ | TBD `docs/strategy/bbi-nav-spec.md` | Reconcile site-architecture-2026-04-25.md §1 vs current `Shop · Brands · Verticals · Our work · Services · About` rendered in landing pages. Decide: 5 items with dropdowns, or 6 items flat. |
| 4 | NAV-2 | Build `bbi-nav.liquid` + `bbi-footer.liquid` snippets | ⬜ | TBD `theme/snippets/bbi-nav.liquid`, `theme/snippets/bbi-footer.liquid` | Single source for nav + footer. Schema for active nav item passed via render context. Uses PB-12's `--snippets` flag to deploy. |
| 5 | NAV-3 | Refactor 10 ds-lp-* sections to render shared snippets | ⬜ | 10 ds-lp-*.liquid files | Replace ~48 lines of embedded header HTML/CSS in each with `{% render 'bbi-nav', active: 'industries' %}` etc. |
| 6 | NAV-4 | Homepage onto shared nav | ⬜ | `theme/layout/theme.liquid`, `templates/index.json` | Extend `bbi_landing` gate to detect `template == 'index'`. Verify homepage renders shared nav, no Starlite leak. |
| 7 | PB-13 | Reconcile brand-dealer (merge or de-gate) | ⬜ | `theme/sections/ds-lp-brand-dealer.liquid` (currently missing on main) | Suffix is in gate but section file is on a separate branch. Either merge or remove from gate. |
| 8 | PB-9 | Extend `bbi_landing` gate to detect collection templates | ⬜ | `theme/layout/theme.liquid` | Add `template.name == 'collection' and template.suffix == 'category'` branch. Test with throwaway template. |
| 9 | PB-10 | Build `collection.category.json` template + `ds-cc-base.liquid` section pattern | ⬜ | TBD `theme/templates/collection.category.json`, `theme/sections/ds-cc-base.liquid` | Mirror ds-lp-* pattern: hero, intro, tile grid via section blocks, breadcrumb, smart-collection "View all" button, ergoCentric/Keilhauer callouts where spec'd, phone CTA. Uses bbi-nav + bbi-footer snippets from NAV-2. |
...
| LEAD-1 | Crawl + dump current lead routing | ⬜ | `docs/plan/bbi-lead-routing.md` | Every CTA → destination mapped. Forms, phone CTAs, email links. |
...
| LEAD-3 | Unify on `bbi-lead-form.liquid` snippet + ack auto-reply per type | ⬜ | TBD `theme/snippets/bbi-lead-form.liquid` | One form, `source_page` + `lead_type` params, single inbox |
...
| CONTENT-1 | Finalize BBI logo asset | ⬜ | `data/logos/bbi-logo-final.png` | `bbi-logo-v2` is Brant Basics wordmark — confirm this is the locked answer or source true BBI wordmark |
...
| ID | Task | Status | Evidence | Notes |
|---|---|---|---|---|
| **LAUNCH-0** | **Image confirmation gate (HARD)** | ⬜ | `data/reports/page-images-audit-<date>.csv` reviewed | Every image on every page enumerated. Steve marks each Approved / Reject / Replace. |
...
| LAUNCH-2 | Manual publish of BBI Landing Dev → live | ⬜ | live theme backup + post-publish snapshot | Backup live theme first |
