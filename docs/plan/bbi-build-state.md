...
| 4 | NAV-1 | Lock canonical nav spec | ⬜ | TBD `docs/strategy/bbi-nav-spec.md` | 🔔 **NEEDS DECISION** — Reconcile site-architecture-2026-04-25.md §1 (5 items: `Shop Furniture · Industries · Brands · Services · About` with mega-menu dropdowns) vs current landing-page rendering (6 items: `Shop · Brands · Verticals · Our work · Services · About` flat). Pick one before proceeding to NAV-2. |
| 5 | NAV-2 | Build `bbi-nav.liquid` + `bbi-footer.liquid` snippets | ⬜ | TBD `theme/snippets/bbi-nav.liquid`, `theme/snippets/bbi-footer.liquid` | Single source for nav + footer. Schema for active nav item passed via render context. Uses PB-12's `--snippets` flag to deploy. |
| 6 | NAV-3 | Refactor 10 ds-lp-* sections to render shared snippets | ⬜ | 10 ds-lp-*.liquid files | Replace ~48 lines of embedded header HTML/CSS in each with `{% render 'bbi-nav', active: 'industries' %}` etc. |
| 7 | NAV-4 | Homepage onto shared nav | ⬜ | `theme/layout/theme.liquid`, `templates/index.json` | Extend `bbi_landing` gate to detect `template == 'index'`. Verify homepage renders shared nav, no Starlite leak. |
