...
| LEAD-1 | Crawl + dump current lead routing | ✅ | commit `TBD` `docs/plan/bbi-lead-routing.md` | Full CTA audit across all 10 ds-lp-* sections, bbi-nav, bbi-footer, ds-cc-base, index.json custom-liquid. Two critical gaps found: (1) design-services uses unreliable `mailto:` form; (2) `/pages/quote` "Online quote form" link goes to `/pages/contact` which has no template. LEAD-3 decisions pre-loaded. Steve to fill in Shopify notification routing + phone line + inbox verification per action items in the doc. |
