...
**Last updated:** 2026-05-08 (Stage 5 audit pass — Wave C rows updated to 🟡 provisional, PB-PDP-1 confirmed ✅)
...
| ID | Task | Status | Evidence | Notes |
|---|---|---|---|---|
| P4-1 | Brands Hub (`/pages/brands`) | 🟡 | `theme/sections/ds-lp-brands.liquid` + `theme/templates/page.brands.json` exist | Section + template in repo. **Shopify Page record + published status unverified** (Wave C built outside worktree — Stage 0 audit finding). Phase 1 API check required. |
| P4-2 | Keilhauer (`/pages/brands-keilhauer`) | 🟡 | `theme/sections/ds-lp-brands-keilhauer.liquid` + `theme/templates/page.brands-keilhauer.json` exist | Same caveat as P4-1 — Page record unverified. |
| P4-3 | Global / Teknion (`/pages/brands-global-teknion`) | 🟡 | `theme/sections/ds-lp-brands-global-teknion.liquid` + `theme/templates/page.brands-global-teknion.json` exist | Same caveat — Page record unverified. |
| P4-4 | ergoCentric (`/pages/brands-ergocentric`) | 🟡 | `theme/sections/ds-lp-brands-ergocentric.liquid` + `theme/templates/page.brands-ergocentric.json` exist | Same caveat — Page record unverified. PB-13 reconciliation resolved (CLEANUP-1 commit `71e7b5e`). |
| P4-5 | About (`/pages/about`) | 🟡 | `theme/sections/ds-lp-about.liquid` + `theme/templates/page.about.json` exist | Same caveat — Page record unverified. |
| P4-6 | Our Work / Portfolio (`/pages/our-work`) | 🟡 | `theme/sections/ds-lp-our-work.liquid` + `theme/templates/page.our-work.json` exist | Same caveat — Page record unverified. Content (48 OCI photos) still to be wired. |
| P4-7 | Contact (`/pages/contact`) | 🟡 | `theme/sections/ds-lp-contact.liquid` + `theme/templates/page.contact.json` exist | Same caveat — Page record unverified. Form routing to sales@ unverified. |
| P4-8 | Delivery & Installation (`/pages/delivery`) | 🟡 | `theme/sections/ds-lp-delivery.liquid` + `theme/templates/page.delivery.json` exist | Same caveat — Page record unverified. |
| P4-9 | Relocation Management (`/pages/relocation`) | 🟡 | `theme/sections/ds-lp-relocation.liquid` + `theme/templates/page.relocation.json` exist | Same caveat — Page record unverified. |
...
| LEAD-3 | Unify on `bbi-lead-form.liquid` snippet + modal + per-type routing + auto-replies | ⬜ | TBD `theme/snippets/bbi-lead-form.liquid` | **Decisions locked 2026-05-07.** Three inboxes: `quotes@`, `design@`, `info@brantbusinessinteriors.com` (Steve forwards all to `steve@` mail-side). Lead-type → inbox: quote→quotes, design→design, contact→info, oecm→quotes. Modal pattern (Web Component + `<dialog>` + focus trap, mirrors NAV-2's `bbi-nav-mobile`). On submit: in-modal success screen with secondary CTA. Auto-reply per type, copy approved by Steve during build. Routing impl: Shopify Flow OR per-form Sender Email OR Cloudflare Worker — picked during LEAD-3. Pre-req: LEAD-INBOX-1 ✅. See `docs/plan/bbi-lead-routing.md`. ~~🔔 NEEDS DECISION~~ resolved. |
