...
- **Collections:** description body is a real CMS field, but only renders if the template's section outputs `{{ collection.description }}` — true for `ds-collection-base` (default), **false** for `ds-cs-base` (`base` suffix) and `ds-cc-base` (custom). SEO title/meta (`seo{}`) always apply regardless of template.
  - **⚠️ SUPERSEDED 2026-06-11 (Step 2.1, doc-PR reconciliation):** the `ds-cs-base` half of this is **no longer true.** Step 2.1 added an answer-first intro to `ds-cs-base.liquid` that **does** render `{{ collection.description }}` (inside `.ds-cs__intro`, **above** the product grid) plus a FAQ band. So `base`-suffix sub-collections now surface their description body (this is what made the Step 2.2 category-copy push possible). `ds-cc-base` (custom category template) remains description-less by design. Original bullet kept for history; treat `ds-cs-base` as **render = yes** going forward.
