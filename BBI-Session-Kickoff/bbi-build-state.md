...
**Current state:** 2026-05-25 (Monday) · **🚀 LAUNCH DAY (Day 11)** · **43 of 54 launch steps done (80%)** · Day 10 (Sun 2026-05-24) EOD COMPLETE — SYS-VERIFY-1 Phase 2 (GO, 0 BLOCKER) + LEAD-2 routing (PASS) both closed as numbered steps; Cornerstone Post 1 LIVE; PR-1 + PR-2 merged (all 4 SYS-VERIFY HIGH findings closed); post-launch monitoring docs shipped. Upwork 137-image package expected to land overnight Sun→Mon → **IMAGE SWAP + LAUNCH-2 today**. The queue below is dependency-ordered with ⏳ ready / 🔒 blocked status flags — no day labels. Fire anything ⏳ ready; 🔒 items become ready as their prerequisites land. Target end-of-Monday LAUNCH-2: ~50+/54 (~93%+) · BBI LIVE. *(This snapshot is the Day 10 EOD sync, written 2026-05-24, facing Day 11.)*

---

## 📍 RIGHT NOW — fireable this moment

⏳ items with no prerequisites blocking them. Pick any, in any order. **Day 11 launch-day morning queue** — clear these, then the Upwork delivery unlocks the IMAGE SWAP → LAUNCH-2 chain.

- **#1 HIGH-3 fix — `product-form-buttons.liquid:30`** — repoint the stale `/pages/contact` CTA to the `/pages/quote` modal trigger (LEAD-2 finding); land this before the IMAGE SWAP session (~30 min)
- **#2 Cornerstone Post 1 visual spot-check** — quick eyeball of the published article on the DEV preview (tables, CTA buttons, captions render clean) (~5 min)
- **#3 Image swap pipeline prep** — pre-draft the Claude Code ingestion prompt + verify the Shopify Files upload pipeline with a test image (~30–45 min)
- **#4 W0-1 + W0-3 final verify** — quick confirm GSC/GA4 + redirects (~10 min, anytime)

> #5 Upwork delivery review unlocks the IMAGE SWAP → SYS-VERIFY re-run → LAUNCH-0→4 chain the moment the 137-image package lands (expected overnight Sun→Mon). The Day 11 EOD tracker + build-state sync (#16) fires at end of day — not a "right now" candidate.

---

## 📋 DO NEXT (linear order) — 16 items, dependency order

No day labels. Order reflects dependencies. Each item carries a status flag (⏳ READY / 🔒 BLOCKED) + time estimate; 🔒 items show a **BLOCKED ON** line. The 9 Day-10 closures (SYS-VERIFY-1 Phase 2, SERP-FIX/SCHEMA PR-1 + merge, Cornerstone Post 1, LEAD-2, W0-2-HOURS, W0-2-PIN, PR-2 + merge, post-launch monitoring docs) have dropped off the queue — see the COMPLETED archive. Reordered for Day 11 launch day: HIGH-3 fix first → image-swap stack → LAUNCH-0→4 stack.

1. **HIGH-3 fix — `product-form-buttons.liquid:30`** — ⏳ READY — ~30 min. Stale `/pages/contact` CTA (LEAD-2 finding) → repoint to `/pages/quote` so the sitewide `bbi-quote-modal.liquid` click handler intercepts it (same pattern PR-2 used to close HIGH-1). Land before the IMAGE SWAP session.
2. **Cornerstone Post 1 visual spot-check** — ⏳ READY — ~5 min. Eyeball the published article on the DEV preview — 3 tables, CTA buttons, `<caption>` + `scope` attrs all render clean under `ds-article.liquid`.
...
10. **Step 38 LAUNCH-1 — GO/NO-GO report** — 🔒 BLOCKED — ~30 min. Pre-publish GO/NO-GO report. **BLOCKED ON:** #9 LAUNCH-0.
11. **Step 39 LAUNCH-2 — GO-LIVE manual click** — 🔒 BLOCKED. The manual Publish click in Shopify Admin; never automated. **BLOCKED ON:** #10 LAUNCH-1.
12. **Step 40 LAUNCH-3 — sitemap resubmit + 72h 404 monitor** — 🔒 BLOCKED — ~15 min. Resubmit sitemap to GSC + start the 72h 404 monitor. **BLOCKED ON:** #11 LAUNCH-2.
13. **Step 41 LAUNCH-4 — post-launch monitoring + mobile smoke test** — 🔒 BLOCKED — ~15 min. Activate the post-launch monitoring playbook (`docs/plan/post-launch-monitoring.md`) + mobile smoke test across Chrome, Safari, Firefox, iOS, Android. **BLOCKED ON:** #11 LAUNCH-2.
14. **GSC-RECRAWL — submit affected URLs after SERP fixes go LIVE** — 🔒 BLOCKED — ~5 min. Submit affected URLs in Google Search Console once the DEV theme is published. Do Monday after LAUNCH-2. **BLOCKED ON:** PR-1 merged (✅ done) + #11 LAUNCH-2 live + 24h re-crawl latency.
15. **Step 36d Cornerstone Post 2 — Healthcare FHTs** — 🔒 BLOCKED — ~2–3h. Same flow as Post 1; links `/pages/healthcare` + `/pages/oecm`. Outline approved by Steve — drafting deferred to **Tuesday**; does not block Monday launch. **BLOCKED ON:** dedicated drafting session (Tue) + Steve draft review.
16. **EOD tracker + build-state update (Day 11)** — ⏳ READY (fire at EOD) — ~30 min. End-of-day sync of both source-of-truth files after LAUNCH-2. Fire at end of day, not a "right now" candidate.
...
- **DATAFORSEO-403** — the DataForSEO MCP returned HTTP 403 during Cornerstone Post 1 drafting (fell back to static keyword targets). Needs a subscription/config fix; SEO-AUDIT-1 is hard-gated on this MCP per `CLAUDE.md`, so this blocks SEO-AUDIT-1. ~30 min triage.
...
- **LEAD-HIGH-2 — no-JS modal fallback** — 9 quote/contact buttons depend on the `bbi-quote-modal.liquid` JS click handler; with JS disabled they reload the current page instead of reaching a form. LEAD-2 judged this NOT lead-loss (graceful, no 404) → deferred. Add a real fallback `href` to all 9 buttons. ~45 min.
