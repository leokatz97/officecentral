...
- **#10 W0-2-PIN** — GBP pin verify + drag if off building (~3 min, Chrome + Leo eyeball)
- **#16 W0-1 + W0-3 final verify** — quick confirm GSC/GA4 + redirects (~10 min, anytime)

> #8 EOD tracker + build-state update is ⏳ ready but only fires at end of day — not a "right now" candidate.

---

## 📋 DO NEXT (linear order) — 21 items, dependency order
...
3. ~~**PR-1 review + merge**~~ ✅ **DONE 2026-05-24** — merged via PR #18 → commit `5df82ce`. PR-2 (HIGH-1 + HIGH-4) shipped same session — branch `feature/quote-form-and-business-furniture-faqs`, PR open. Scope: HIGH-1 (quote→contact form gap closed via 2-line default-string flip in `ds-lp-quote.liquid:402,403` — all 4 broken CTAs on Quote page now route to `/pages/quote` which the sitewide `bbi-quote-modal.liquid` global click handler intercepts to open the existing modal contact form; hero CTA, Online quote form channel, OECM callout CTA, and closer CTA all fixed by the same 2 lines) + HIGH-4 (added 4 procurement-actionable parent-level FAQs to `collection.business-furniture.json` — OECM Agreement coverage across categories, multi-category quote workflow, multi-category install scope with after-hours hedging, parent-level free space planning; FAQPage JSON-LD auto-emits via `ds-cc-base.liquid` mirroring AI-9 pattern) + sameAs TODO comment in `bbi-localbusiness-schema.liquid` rewritten to reflect Steve's 2026-05-24 confirmation that BBI has no current LinkedIn/Facebook/Instagram presence.
4. ~~**Step 36c Cornerstone Post 1 — OECM for Ontario School Boards**~~ ✅ **DONE 2026-05-24** — article published to LIVE Shopify Admin at `/blogs/news/oecm-ontario-school-boards-office-furniture` (article ID `689003888953`, blog `news` `id=108557861177`). 2,446 words / 16,600 chars; 3 HTML comparison tables (Direct Award vs Open RFP, Eligible Buyer Sectors, Coverage Under 2025-470); 1 numbered 7-step procurement workflow with step/responsibility/output metadata; 6 procurement-actionable Q&As; 8 internal cross-links (`/pages/oecm` ×3, `/pages/quote` ×2, `/pages/education`, `/pages/design-services`, `/collections/seating`, `/collections/desks`) — within 5–8 target. Featured image: none (Option A v1 fallback per HALT 0 — `data/page-images/` is AI-generated per `project_ai_image_pipeline.md`, prompt hard-rule forbids AI photos; real photo added in follow-up edit after Upwork delivery + Step 46 IMAGE SWAP). Published immediately to LIVE per Leo HALT 3A decision (Steve review deferred to after-publish — normal post-publish editorial pass, not a tracked blocker). DataForSEO MCP returned HTTP 403 — fell back to static KW targets per prompt CONTEXT. BlogPosting JSON-LD baked into `ds-article.liquid:130-168` from Day 9 H-1 batch-fix — auto-emits on LAUNCH-2 Monday when DEV theme publishes; LIVE Avada theme currently emits 0 JSON-LD on the article URL (verified). Voice-rule audit clean: 0 literal "BBI" in customer copy, 0 forbidden lead-time patterns, full "Brant Business Interiors" customer-facing throughout, "Brant Basics" cited in OECM legal-entity context, family-owned since 1964 + 296 George St N Peterborough in closing section. All claims sourced from `ds-lp-oecm.liquid` + `page.oecm.json` + bbi-build-state Day 8 verified facts — zero invented stats. Draft archived to `data/drafts/cornerstone-post-1-oecm-school-boards-v2.html`; API response to `data/backups/articles/cornerstone-post-1-response-20260523-204502.json`.
...
7. **Image swap pipeline prep** — ⏳ READY — ~30–45 min. Pre-draft the Claude Code prompt for Monday image ingestion + verify the Shopify Files upload pipeline with a test image.
8. **EOD tracker + build-state update** — ⏳ READY — ~30 min. End-of-day sync of both source-of-truth files. Fire at end of day, not a "right now" candidate.
9. **W0-2-HOURS — GBP hours fix Mon–Fri 9–5 ET** — ⏳ READY (Chrome-driven) — ~2 min. Correct Google Business Profile hours to the canonical 9–5 ET.
10. **W0-2-PIN — GBP pin verify + drag if off building** — ⏳ READY (Chrome + Leo eyeball) — ~3 min. Confirm the map pin sits on 296 George St N; drag if off.
11. **W0-2-PHOTOS — 10 photos upload to GBP** — 🔒 BLOCKED — ~15 min. Upload 10 photos to the Google Business Profile. **BLOCKED ON:** Steve photo selection (10 from `oci-photos` + `design-photos` folders).
12. **GSC-RECRAWL — submit affected URLs after SERP fixes deploy** — 🔒 BLOCKED — ~5 min. Submit the affected URLs in Google Search Console once the SERP fixes are live. **BLOCKED ON:** SERP-FIX PR merged (#3) + 24h re-crawl latency.
13. **Upwork delivery review** — 🔒 BLOCKED — ~15–30 min. Final QA of the manifest CSV + images. **BLOCKED ON:** Upwork manifest CSV + 137 images — EOD Sunday May 24.
14. **Step 46 IMAGE SWAP session** — 🔒 BLOCKED — ~2–3h. Claude Code ingests the manifest + uploads to Shopify Files + maps to template slots across all pages. Closes IMAGE-SOURCING-V2 ✅ when all 137 images live in DEV. **BLOCKED ON:** #13 Upwork delivery.
15. **SYS-VERIFY-1 Phase 2 re-run** — 🔒 BLOCKED — ~30 min (light). Verify no regressions after the image swap. **BLOCKED ON:** #14 image swap complete.
16. **W0-1 + W0-3 final verify** — ⏳ READY anytime — ~10 min. Quick confirm GSC + GA4 (✅ done 2026-05-22) and redirects CSV (✅ 171/173 verified). Quick-confirm only.
17. **Step 37 LAUNCH-0 — image confirmation gate** — 🔒 BLOCKED — ~30 min. Leo row-by-row image CSV approval; hard gate before LAUNCH-1. **BLOCKED ON:** #15 SYS-VERIFY-1 re-run.
18. **Step 38 LAUNCH-1 — GO/NO-GO report** — 🔒 BLOCKED — ~30 min. Pre-publish GO/NO-GO report. **BLOCKED ON:** #17 LAUNCH-0.
19. **Step 39 LAUNCH-2 — GO-LIVE manual click** — 🔒 BLOCKED. The manual Publish click in Shopify Admin; never automated. **BLOCKED ON:** #18 LAUNCH-1.
20. **Step 40 LAUNCH-3 — sitemap + 72h 404 monitor** — 🔒 BLOCKED — ~30 min. Submit sitemap to GSC + start the 72h 404 monitor. **BLOCKED ON:** #19 LAUNCH-2.
21. **Step 41 LAUNCH-4 — mobile smoke test** — 🔒 BLOCKED — ~30 min. Mobile smoke test across Chrome, Safari, Firefox, iOS, Android. **BLOCKED ON:** #19 LAUNCH-2.
...
**Pending now:**
- **Idea #15 — 3-card SKU picks** (OTG / GFG / Heartwood) — needed before #14 image swap.
- **Cornerstone Post 1 after-publish review** — post is LIVE at `/blogs/news/oecm-ontario-school-boards-office-furniture` (article ID `689003888953`); Steve eyeballs and edits in Shopify Admin if needed. No longer blocking — #4 is closed.
- **Cornerstone Post 2 review** — needed for #5 to complete.
- **W0-2-PHOTOS selection** — 10 photos from `oci-photos` + `design-photos` folders — needed for #11.
...
> Resolved/closed Steve items moved to record: **W0-1** ✅ done 2026-05-22 (GA4 `G-XLCM9LCNLN` + GSC domain property + GSC↔GA4 link; Leo added as GA4 Admin) · **W0-3** ✅ verified Day 9 (171/173 redirects live) · **INSTALL-PHOTO-QUERY (Step 53)** ✅ answered 2026-05-21 (stock fine) · **STEVE-FACT-CHECK (Step 55)** ✅ resolved 2026-05-21 · **LEAD-INBOX-1 (Step 21)** ✅ done 2026-05-14 · **SCHEMA-LOCALBIZ-1 sameAs URLs** ✅ answered 2026-05-24 (no current LinkedIn/Facebook/Instagram presence — sameAs stays empty; comment in `bbi-localbusiness-schema.liquid` updated to reflect this in PR-2). LEAD-3 manual M365 follow-ups remain (see `docs/strategy/bbi-lead-routing.md`). Leo launch gates (LAUNCH-0 #17, LAUNCH-2 #19) live in the DO NEXT queue; the INTERLINK-3 INFO pre/post-launch call sits in the POST-LAUNCH BACKLOG.
...
**Day 10 · 2026-05-24 — 1 numbered step (+ 6 non-counted closures)**
...
- Step 35 SEO-AUDIT-1 — technical SEO audit, 🔒 blocked on DataForSEO MCP connectivity
