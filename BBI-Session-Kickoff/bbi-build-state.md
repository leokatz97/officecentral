...
- **SCHEMA-CRIT-1 — PARTIAL ✅ Fix 1 + CRIT-1b shipped & RRT-confirmed; CRIT-1c (returns/shipping) remains Steve-gated** (branches `feature/schema-crit-1-2026-05-27`, `feature/schema-crit-1b-2026-05-28`):
  ...
  - **SCHEMA-CRIT-1c — returns/shipping policy fields (Steve-gated) — REMAINS.** Add `hasMerchantReturnPolicy` (`MerchantReturnPolicy`) + `shippingDetails` (`OfferShippingDetails`, Ontario region + "Contact for quote" handling) to PDP `offers`. **Blocked on Steve confirming return + shipping policies** and the policy pages going live. These are the other half of full Merchant Listing eligibility (with CRIT-1b's two fields). ~15 min once policies are decided.
...
- **SCHEMA-H-1 🆕** (added 2026-05-28 from SCHEMA-CRIT-2 wrap; same `@id`-cleanup class as SCHEMA-POLISH-1, ~20-30 min) — convert the inline `provider` LocalBusiness redeclarations in `theme/sections/ds-lp-oecm.liquid` (~line 304-317) and `theme/sections/ds-lp-quote.liquid` (~line 363-376) to `@id` references to `#organization` (matching the `ds-lp-delivery.liquid` / new `bbi-service-jsonld.liquid` pattern). Eliminates the F-4 duplicate-entity WARN — currently each of those two pages emits a 3rd LocalBusiness node inline as the GovernmentService/Service `provider` instead of referencing the existing `#organization`/`#localbusiness` entity. **This is an edit to live, already-shipped schema** (not additive) — deliberately kept out of SCHEMA-CRIT-2 (which was purely additive net-new emitters) to keep blast radius scoped. **Confirmed still outstanding** via the SCHEMA-CRIT-2 RRT spot-check — surfaces as the non-critical LocalBusiness issue on the affected pages. Standard approval gate + sha discipline. Could be bundled into a future SCHEMA-POLISH-1 session (same fix class).
