...
## 🟢 Day 16 — 2026-05-29 — SCHEMA-BLOG-1 → BLOG SCHEMA ON `/blogs/news` + BLOGPOSTING ENHANCEMENT (H-2/F-11 RESOLVED — SOLO-ACTIONABLE SCHEMA LANE NOW COMPLETE) · SCHEMA-BRAND-1 → 7 BRAND ENTITIES ON 6 MANUFACTURER PAGES (F-13/H-3 RESOLVED) · F-LOCALBUSINESS-IMAGE → `image` FIELD ON BOTH CHROME LOCALBUSINESS NODES (RESOLVED)
...
- **SCHEMA-CRIT-2 — COMPLETE ✅ shipped 2026-05-28** (branch `feature/schema-crit-2-2026-05-28`, off `feature/lcp-1a-hero-srcset-2026-05-27` @ `36360e2` — PR #32 tip; PR #33, commit `97b9416`):
...
- **SCHEMA-CRIT-3 — COMPLETE ✅ shipped 2026-05-28** (branch `feature/schema-crit-3-2026-05-28`, off `feature/schema-crit-2-2026-05-28` @ `7f205da` — PR #33 tip; stacked **PR #34**, commit `cee7f57`):
...
- **HOTFIX-MOBILE-LCP-1 🆕🔥** (Tier 1, **highest-acquisition-impact open item**, added 2026-05-27 late-night from QW-3 verification) — Mobile performance is genuinely failing on LIVE. Core Web Vitals → ranking is a more direct signal than rich-result eligibility, and the gap is enormous (LCP 13.3s vs Good threshold 2.5s = 5.3x over). **⚠️ REFRAME 2026-05-28 (RENDER-BLOCKING-1 diagnosis):** the headline LCP figures (13.3s baseline, "5.0s", "11.9s") are **Lantern simulation estimates**, not real paint — trace-observed LCP is 0.4–2.3s and the hero downloads <600ms every run. The real, measurable problem is **interactivity (TBT 134→3267ms)**, owned by sub-item 1b. No CrUX field data exists (site below traffic threshold), so lab LCP is the least trustworthy signal — track TBT, revisit LCP when field data appears. 1a-HERO-SRCSET shipped (confirmed working); RENDER-BLOCKING-1 diagnosed & ruled out; **1b is the sole remaining lever.**
  ...
  **Remaining scope: fix-phase only, split into three discrete sub-items.** Diagnostic work that would have been the first ~30 min of this item is already complete (PageSpeed surfaced everything). Each sub-item below is independently shippable on Day 14+ based on time/energy:
  ...
  - **HOTFIX-MOBILE-LCP-1b — JS optimization (~90-120 min) — 🔥 NOW THE PRIMARY REMAINING MOBILE-PERF LEVER (reframed 2026-05-28 from RENDER-BLOCKING-1 diagnosis).** Addresses diagnostic signals #2 (Legacy JavaScript, 19 KiB), #4 (Forced reflow), and #6 (Reduce unused JavaScript, 334 KiB). Fix surfaces: identify dead/unused theme JS bundles; modernize transpile targets to drop ES5 polyfills for modern browsers; profile forced-reflow source in Chrome DevTools Performance panel; defer/async non-critical scripts; consider code-splitting per template (PDP-only JS not loaded on homepage, etc.). **⚠️ Reframed real-user impact:** the harm is on **INTERACTIVITY (TBT/INP), not hero paint.** Day 14 diagnosis proved the hero paints ~0.4–2.3s observed in every run (srcset fix solid); the bimodal "12s LCP" is a Lantern simulation artifact driven by main-thread JS variance (TBT swings 134ms→3267ms, ~1.3s script eval+parse). **Measure success via TBT, not lab LCP** — there's no CrUX field data (site below traffic threshold), so the simulated lab LCP is the least trustworthy signal we have; TBT is real and measurable. Originally "Recommended SECOND after 1a"; 1a-HERO-SRCSET shipped + RENDER-BLOCKING-1 ruled out, so 1b is now the standalone next target.
