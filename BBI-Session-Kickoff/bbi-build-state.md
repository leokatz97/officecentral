...
| SPEC-HERO-PUSH | Hero 100 spec gap-fill + metafield push | 🟡 | `data/specs.json` (100) + `data/logs/pe2-push-20260511-210351.json` + `data/logs/pe2-push-20260511-211345.json` | **Two pushes completed 2026-05-11.** Push 1: 75 products / 174 fields (finishes, key_features, certifications, model_codes). Push 2 (after auto-patch): 75 products / 217 fields — 33 OTG/Global products auto-patched warranty + country_of_manufacture via `scripts/merge-hero-specs.py`. **Remaining gaps requiring sessions:** 23 fully empty products (H1A + H1B), 19 significant gaps (H2), ~35 minor gaps (H3). Session files at `BBI-Session-Kickoff/hero-spec-prompts/`. After each session: `python3 scripts/merge-hero-specs.py --live --push` |
| HERO-SPEC-SESSIONS | Hero 100 spec gap enrichment sessions (H1A → H3) | 🚧 | `BBI-Session-Kickoff/hero-spec-prompts/` — 4 batch files + MASTER-INSTRUCTIONS + `scripts/merge-hero-specs.py` | **Blocked on Steve running sessions.** Infrastructure complete: 4 session files with inline missing-field lists, brand hints, and auto-merge/push end-of-session block. Coverage after auto-patch: 33/99 gap products resolved. Remaining: H1A (12 empty desks/tables), H1B (11 empty remaining incl. installation1000 + outdoor), H2 (19 significant gaps), H3 (~35 minor gaps). |
| PE-PASS-3 | Push enrichment to Shopify (descriptions + specs + vendor) | 🚧 | `scripts/push-pe3-enrichment.py` written | **Blocked on enrichment sessions completing.** Script built: reads `data/reports/pe-pass2-output.json` (enrichment 157) or `data/specs.json` (Hero 100 via `--hero` flag), diffs against current Shopify state, writes `body_html` + 12 `specs.*` metafields + optional vendor override. Dry-run default. Two modes: `python3 scripts/push-pe3-enrichment.py --live` (enrichment 157) and `python3 scripts/push-pe3-enrichment.py --hero --live` (Hero 100 spec re-push). After enrichment sessions done: run both modes. |
