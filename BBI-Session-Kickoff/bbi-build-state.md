...
1. ✅ **Post 1 schema — DONE (2026-06-02).** Article + FAQPage JSON-LD shipped on the LIVE Post 1 (`oecm-ontario-school-boards-office-furniture`). **Article** (`BlogPosting`) was already complete + live — it's **template-level** in `ds-article.liquid` (auto-emits on every article: headline/description/articleBody/datePublished/dateModified/author/publisher=Brant Business Interiors+logo/image/articleSection+keywords) → **no action needed, free for every future post**. **FAQPage** added via the `faq.items` article metafield (Admin-API only, **no theme write / no watcher**): 6 **verbatim** on-page Q&A from the visible "Common Questions" section, written as `Question||Answer` (`list.single_line_text_field`, byte-readback EXACT MATCH, 0 userErrors). **Leo-validated:** schema.org validator FAQPage 0 errors/0 warnings + BlogPosting 0 errors; Google Rich Results Test detects valid Article + valid FAQ. Honest caveat: FAQPage earns no SERP rich result for a non-gov/health site (entity-graph / AI-Overview grounding value only). **🔑 Reusable cornerstone-schema mechanism:** Article schema is template-level (free for every post); each cornerstone only needs `faq.items` populated **where it has real, visible on-page Q&A** (Google requires markup to mirror the visible section — never fabricate). This is the proven model for Blog #2+.
2. **Blog #2 — P1 design/space-planning cornerstone** (Leo) — lead production focus per the locked roadmap; proven model from Post 1. **🆕 PUBLISH-PREPPED + SOURCED** (`data/content-drafts/01-office-space-planning-pillar.md`, PR `feature/post1-publish-prep-2026-06-03`, 2026-06-03). Every stat fact-checked + cited inline (CCOHS/Govt-of-Manitoba 6.7 m²/72 ft² workstation — *attribution corrected: CCOHS cites Manitoba, not its own standard*; Aquila 150–250 ft²/person; Archie 20–25 ft²/seat); all ⚠️ flags resolved (founding-1964 + Peterborough HQ confirmed vs live Post 1; "four basic layouts" softened — no single authoritative list). **PUBLISH PACK appended** (final title<60 + meta + `faq.items` 6 pairs *byte-verified verbatim* vs visible Common Questions + schema note + Steve-image flags + D3 link spec HELD + 10-step publish checklist mirroring Post 1: `POST /blogs/108557861177/articles.json` → `faq.items` metafield → Steve featured image → validate). **🆕 VOICE-EDITS APPLIED (2026-06-03, PR `feature/post1-voice-edits-2026-06-03`, DOCS-ONLY):** bounded edit pass on `01-office-space-planning-pillar.md` — (1) **every em-dash removed** and rewritten to natural punctuation (one exception left intact: the em-dash inside the backticked *locked title suffix* quote, flagged in-draft as verbatim template output, not body copy); (2) **6 Decisions-for-Leo resolved** — density worked example **moved 1,000 ft² → 2,000–2,500 ft²** and reframed as a teaching contrast (bare 72 ft²/desk math ≈ 30-plus desks vs. sourced 150–250 ft²/person whole-office density ≈ 10–15 people; verified internally consistent vs. the benchmark table + sources), OECM line upgraded to the cornerstone's exact phrasing ("registered under our parent legal entity, Brant Basics, as an authorized OECM Supplier Partner"), layout taxonomy "zoned" → **"activity-based"** (Leo's traffic call: kept "Hybrid" + "Activity-based / neighbourhood" as two distinct searchable patterns), founding-1964 / geo-scope / 150–250 range all confirmed no-change; (3) **`faq.items` re-synced** byte-for-byte across all 3 locations (visible Common Questions = numbered list = PUBLISH PACK §2; set-comparison PASS). ⚠️ **One pre-existing flag surfaced (NOT caused by em-dash removal):** the meta description is **161 chars** (over the ≤156 target — was mislabeled "156"); title corrected 49→**46**; a compliant 152-char rewrite is proposed in-draft but **not applied** — editorial call for Leo. **Remaining: Leo's final review (incl. meta-trim decision) → publish.** *(Article/BlogPosting schema is template-level in `ds-article.liquid` → no action.)*
3. **Roadmap cadence — ~5 more posts** (Leo) — **ergonomics FAQ hub early for AEO** (winnable question/PAA vein, NOT high-KD head terms like "ergonomic office chair"/"standing desk"), then the P2 cornerstones (Post 2 Healthcare/FHT, Post 3 Cubicle-vs-Open-Plan municipal) with P3 + P4 geo/buying interleaved. Article schema on all; FAQPage on question-format clusters. **🆕 CONTENT ENGINE — batch-1 drafts READY (overnight 2026-06-03, DOCS-ONLY PR):** resources `data/reports/content-engine/` (north-star-icp.md · topic-bank.md · keyword-serp-research-2026-06-03.md = fresh DataForSEO pull) + **5 first-draft posts** in `data/content-drafts/` (index `00-INDEX.md`): (1) office-layout pillar, (2) ergonomic-chairs-worth-it AEO/FAQ hub, (3) quiet-spaces/acoustic-pods, (4) canadian-made-office-furniture, (5) hot-desking/desk-allocation. Each = keyword map + competitor gap + full draft + FAQ-as-`Question||Answer` (drops into `faq.items`) + D3 link spec (HELD) + meta. Drafts #3/#4 gate to their collection builds (#4/#5 below); 12 "Questions for Leo" parked in the index (top fact-check: Draft 4 Canadian-made origins — verify per manufacturer before publishing).
...
**SEO / AEO — also pending:** **D3** 85-PDP internal-linking pass + inbound links **TO Cornerstone Post 1** (**HELD** until Tier 1 collection + exec-desks URLs final — unblocked once #4–#6 ship; the biggest internal-linking lever) · brand-page→collection wiring (anti-orphan) · 4 rescues (repopulate + internal-link rank-bleeding collections) · **sitewide org-schema warning (non-critical, pre-existing, NOT from the FAQ work)** — schema validation flagged `addressCountry: CA` set on an `areaServed` object typed as `State` (Ontario), which schema.org doesn't recognize for `State`; **0 errors, warning only**. Fix later by not emitting `addressCountry` on the `State`-typed `areaServed`.
...
**Day-20 detail (2026-06-02):** **Day 20 (Phase 3) — BRAND-FILTER BBI SUPPRESSION + ⚠️ FACET-RENDER STILL DOWN (fix deployed, doesn't render)** (OPEN PR, theme write · role=main `186373570873` · SCHEMA-CRIT-1 PASS, no watcher): shipped dealer-name suppression in `ds-cs-base.liquid` — the `vendor_list` builder now skips any vendor containing "brant business interiors" (catches **`Brant Business Interiors` 119** + **`Office Central & Brant Business Interiors` 4** = 123 products), so no dealer name can surface as a Brand chip; cards keep `data-vendor`, so the residual stays browsable, just not brand-filterable. Admin-API asset readback **byte-for-byte MATCH** on role=main. **⚠️ FACET-RENDER STILL DOWN — fix DEPLOYED but does NOT render for users (same OPEN PR):** `push`→`concat` + card-label suppression are **DEPLOYED to the live theme (role=main) + byte-verified** (Admin-API MATCH `0fff77a4fcf98b15`), **BUT the Brand/Type/Room facets still do NOT render for real users** — confirmed in **Leo's real browser, including the cart-cookie cache-bypass**. CC's earlier "✅ resolved / Brand=5 fresh-render confirmed" was a **FALSE POSITIVE** (the automated curl / Section-Rendering-API / cart-cookie reads were false-positive-prone throughout this episode; the only authoritative check is a real browser). `push`→`concat` IS the correct Shopify idiom, but **why it still doesn't render is UNCONFIRMED** — candidates: array initialization (`'' | split: ','`), a deeper render-context issue, or the diagnosis needs a real-browser-confirmed redo. **Filter system (Brand/Type/Room + the planned Made-in-Canada filter) remains DOWN → DEFERRED, non-harmful** (filters never rendered → zero regression; cards + Price unaffected). #88 dealer-name suppression logic is correct + deployed (no-op until facets render). 50-cap remains theoretical/moot regardless (all 108 `base` collections ≤50 products). **LESSON: do NOT self-certify renders — real-browser only.** See the **Day-20 (Phase 3 Brand filter)** section below + `data/reports/brand-filter-bbi-suppression-2026-06-02.md`. · **Day 20 — CATALOG: 225 vendor=BBI RE-SOURCING** (OPEN PR, ✅ EXECUTED): scanned the 221 live `vendor=BBI` data-errors → **115 CONFIDENT** (97 tier-1 + 18 tier-2) / **87 AMBIGUOUS** / **19 SKIP**; **hold lifted 2026-06-02 — 97 tier-1 + 5 ambiguous WRITTEN = full 225-resolution complete** (97/97 PASS, hardened readback MATCH). At write time the staged decoder names were **aligned to existing live chips** to avoid fragmentation: Heartwood Manufacturing 40 (→79) / IOF 26 (new) / Richelieu 12 (new) / Horizon 10 (→15) / MityBilt 9 (→11). Independent readback: **BBI 216→119**. Remaining-BBI for chip-suppression scope = **119** (18 tier-2 + 82 ambiguous + 19 services). See the **Day-20 re-sourcing** section + `data/reports/bbi-resourcing-provenance-2026-06-02.md`. · **Day 20 — EXEC-DESKS COLLECTION BUILT + PUBLISHED + SURFACED** (OPEN PR): captures `executive desk` (~1,300/mo, KD0). Live-Admin recon confirmed the `executive-desks→desks` redirect **ABSENT** (handle free to serve 200) but an empty UNPUBLISHED **custom** collection (id `526867005753`) squatting the handle. Found the tag schema **mid-migration/fragmented** (`type:desks` 38 vs `type:desk` 7 vs `type:workstation`; `segment:executive-office` on **1** product) **and** every sibling desk sub-collection (`l-shape-desks`/`u-shape-desks`/`straight-desks`/`office-suites-desks`/`height-adjustable-tables`) is **manual** → pivoted from smart-rule to **manual curation** (matches siblings, no tag pollution of a broken schema). Reused the squatting custom collection: added **30** curated Tier-1 executive-grade desks (L/U-shape, double-pedestal, suites, executive-named, big workstations), `sort=best-selling`, SEO meta (`title_tag` 50ch + `description_tag` 158ch, full "Brant Business Interiors" so the R5 suffix auto-suppresses), published. **Hardened readback:** GraphQL productsCount=**30**, `published_at` set, redirect still ABSENT, storefront **200** (no 301). **Phase 2 nav (theme write · role=main `186373570873` · SCHEMA-CRIT-1 PASS, no watcher):** repointed the existing `tile-executive-desks` in `collection.desks.json` from `/collections/office-suites-desks`→`/collections/executive-desks`, title "Executive Desk Suites"→"Executive Desks"; Admin-API asset readback MATCH + cache-busted render confirms `href="/collections/executive-desks"` on the live Desks page. Script: `scripts/build-exec-desks-collection.py` (dry-run default, backup+log to gitignored `data/`). **IMAGE-GEN follow-up:** the tile reuses the existing `bbi-coll-img-desks-tile-executive-desks-v4.jpg` (originally made for the office-suites context) — optional fresh art for the exec-desks set, non-blocking. **Rank to watch:** `executive desk` (~1,300/mo) in the next snapshot. **Hygiene follow-up:** prune the dead `/collections/executive-desks,/collections/desks` row from `data/redirects/landing-refresh-301s.csv` (no longer applied live). · **Day 20 (Phase 3) — COLLECTIONS NAV PROMOTION + desks.json REPOINT + QUIET SPACES NAV-HOP FIX** (OPEN PR, theme write): SCHEMA-CRIT-1 PASS; 10 files PUT to role=main with hardened-readback MATCH. Nav is hardcoded (`bbi-nav`/`bbi-footer`), not a linklist → CC theme write. Surfaced 16 promotable handles as sub-collection tiles (proven `desks.json` pattern) + footer gaming link; repointed 2 dangling desks tiles (unblocks batch-2); fixed the Quiet Spaces card-hop → Phone Booths (`telephone-booths`). **batch-2 CSV import stays gated on this PR merging + going live.** See the **Day-20 (Phase 3)** section directly below + `data/reports/collections-arch-phase3-nav-promotion-2026-06-02.md`. · **Day 20 — COLLECTIONS PUBLISH-STATE RECONCILIATION** (OPEN PR): Admin-API publish toggles only (0 theme/redirect/menu writes). Locked the **dormant-redirect mechanic** (a `/collections/<h>` redirect is inert until `<h>` is unpublished). Executed 5 writes, all hardened-readback MATCH — **A:** republished 4 rank-bleeding collections (`benching-desks` #72, `coat-racks-accessories` #34, `modesty-panels` #36, `picnic-tables` #48 → serve own 200, rank reclaimable, thin→repopulate later); **B:** unpublished `reception` → now 301→`reception-desks-desks` (3 products ⊆ canonical's 9, completes Day-19 fold). `healthcare-seating` already done (no-op). **Decisions:** boardroom-conference-meeting = KEEP-CANONICAL (ranks #22 vs rank-less nav `boardroom`; Steve drops the redirect); keilhauer = HOLD. **Gated carry-forward ✅ DONE (follow-up, 2026-06-02, OPEN PR):** Steve imported `collections-phase2-batch1-301s.csv`; the gated batch-1 source-unpublish ran — **33 sources unpublished, 33/33 hardened-readback MATCH + 301 firing**, keilhauer held. Batch-1 consolidation now LIVE. See the **Day-20 follow-up** section + `data/reports/collections-batch1-source-unpublish-2026-06-02.md`. · **Day 19 close-out:** PRs **#72–#76 all MERGED** to `main` (`798efee`); no open PRs from this work (the unrelated #1/#2/#5 are untouched). Post-S6 close-out landed **#75 (GFG warranty default → `null`** — kills the over-stamp trap at source), plus three read-only analysis artifacts + one Admin-API execution surface, **0 theme files touched**: **COLLECTIONS-AUDIT** (358 collections / nav exposes 9 / ~349 orphans / ~120 empty-but-200 / 3 ranking-but-empty / vendor 66% brand-filter-ready — 4 decisions to the architecture call); **LANDING-REFRESH-PLAN + SURFACE A** (D1 soundness-gate killed 3 bad premises; migrated reception meta → `reception-desks-desks` canonical, repaired `/collections/healthcare` meta, unpublished empty `healthcare-seating`, shipped a 4-row 301 CSV for **Steve to import manually**); **SITE-HEALTH REGRESSION** (no regression introduced — 53/53 PDPs clean; 5 action items all in the cannibalization/redirect layer). Carry-forward: Steve imports the 301 CSV (boardroom row = higher-risk, watch next rank snapshot); architecture web-chat call (4 decisions); deferred exec-desks build + D3 PDP-link wiring; LANDING-PAGE-THEME-CONTENT-REFRESH theme write. · **Day 19 — PHASE-A-BLOCK-4-SESSION-6** shipped **26 Global-family PDPs enriched** (PR #74, ✅ **MERGED**) — cluster-weighted on waiting-room/lounge seating + agent-corrected boardroom picks (Craft modular units = lounge poufs/wedges, **not** conference tables; work-table = OTG value table). **Warranty SOURCE-OR-EMPTY discipline held clean: 0 lifetime over-stamps** — all 9 value/MVL items → 2yr/5yr; chevron caught + corrected lifetime→2yr (Basics value line); DTS occasional tables → 5yr value; GFG-premium seating → sourced lifetime+5yr. 26/26 hardened readback MATCH, theme untouched (Admin-API product writes). Honest KW accounting: 6 genuine `office chairs for waiting room` landers; 0 forced boardroom/exec-desk. Block 4 enrichment total now **85 products** (S1-6). · **Day 19 — PHASE-A-BLOCK-4-SESSION-5** shipped **27 Global-family PDPs enriched** with the 13-field `specs.*` framework + priority-keyword integration (PR #73, **merged**). First enrichment session to apply the `priority-keywords.yaml` v1 lock to PDPs, validate **officestogo.com as the Newland source** (5/5), and the **sub-brand→Global parent** routing (all 27 → manufacturer = Global Furniture Group). Surfaced + fixed a **batch-wide warranty over-stamping** issue (agents defaulted "Limited Lifetime" on 14 metafields + 17 bodies) by re-sourcing both manufacturer warranty pages and applying precise per-catalog terms. Block 4 enrichment total now **59 products** (Sessions 1-5). · Day 18 shipped **5 PRs (#66–#70)** + reconciliation (#71) + rank-tracking (#72). See the **Day 19** and **Day 18** sections below. *(The launch-day snapshot that previously occupied this line is preserved verbatim in the next paragraph for history.)*
...
## ⚠️ Day 20 — 2026-06-02 — Phase 3 Brand filter: BBI suppression shipped · facet-render fix DEPLOYED but STILL DOWN for users · DEFERRED · MERGED

**Mechanism (Phase 0 finding).** BBI collection-page filters are **not** Shopify Search & Discovery — they are a **theme-native client-side JS facet** in `theme/sections/ds-cs-base.liquid` (base for all sub-collection pages). The Brand facet's source is the raw Shopify **`product.vendor`** field (the `vendor_list` builder, lines 352–356); checkboxes render at lines 413–432; JS toggles `.ds-cs__card--hidden` by `data-vendor` (lines 668–684). No S&D, no metafield.

**Source-field choice — raw vendor, not `brand:*` tags.** Live Admin-API readback (657 products): vendor field is present on **100%** of products and is now clean post-resourcing; `brand:*` tags cover only **525/657 (132 untagged would vanish)**, Teknion has no standalone `brand:teknion` (only legacy `brand:global-teknion`), AND `brand:brant-business-interiors` still exists on **7** products — so tags do **not** auto-suppress BBI as the original Phase-3 hypothesis assumed. Raw vendor is the complete signal and is the field the existing mechanism already reads → keep it; suppress by string.

**Suppression shipped (Leo-approved, substring).** Edited the `vendor_list` builder to skip any vendor whose downcased name contains `brant business interiors`. Catches both dealer strings: **`Brant Business Interiors` (119)** + **`Office Central & Brant Business Interiors` (4)** = 123 products. Cards still render `data-vendor` → residual products stay **browsable** (visible when no brand is filtered), just have no Brand checkbox → **not brand-filterable**. Exactly the spec.

**Deploy + hardened readback (the gate).** PUT `sections/ds-cs-base.liquid` to role=main `186373570873` (verified `role=main` by `GET /themes.json`; no `shopify theme dev` watcher running → SCHEMA-CRIT-1 PASS). Admin-API asset readback: suppression line present, old unconditional push gone (count 1), **byte-for-byte MATCH** local↔remote (36,643 bytes).

**⚠️ NOT RESOLVED — fix DEPLOYED + byte-verified, but facets STILL DO NOT RENDER for users (2026-06-02; #88 MERGED with this corrected note).** `push`→`concat` (the 3 appends) + card-label dealer-name suppression are deployed to the live theme (role=main) and Admin-API byte-verified (`0fff77a4fcf98b15`), **BUT Brand/Type/Room facets still do not render** — confirmed in **Leo's real browser, including the cart-cookie cache-bypass**. CC's earlier "✅ resolved / fresh-render confirmed Brand=5" was a **FALSE POSITIVE**: the automated render reads (curl / Section-Rendering-API / cart-cookie) were false-positive-prone throughout this episode and were refuted by the authoritative real-browser check. `push`→`concat` IS the correct Shopify idiom, so it was a necessary fix, but **why the facets still don't render is UNCONFIRMED** — candidates: array initialization (`'' | split: ','` producing an unexpected base array), a deeper render-context issue, or the whole diagnosis needs a real-browser-confirmed redo. **The filter system (Brand/Type/Room + the planned Made-in-Canada filter) remains DOWN → DEFERRED, non-harmful** (the facets never rendered, so there is ZERO regression; product cards + Price filter are unaffected). The original `push` ≠ Shopify-filter finding still stands as one real defect (it appeared in exactly these 3 lines theme-wide; `concat` is the supported idiom) — it just wasn't the *whole* cause. **LESSON: CC's automated render reads cannot certify a render; the only authoritative check is a real browser. Do NOT self-certify renders.**
- **Fix A** — 3 `| push:` → `concat`-via-`split` idiom (`assign _one = x | split: '~~|~~'` → `concat: _one`). Restores Brand + Type + Room together.
- **Fix B** — card vendor label now suppresses the dealer name too (mirrors the facet substring at line ~363) → "Brant Business Interiors" no longer leaks on product-card brand labels.
- **Brand 50-cap = theoretical, deferred.** All 108 `base`-suffix collections: max=**50** products, median 5, **zero exceed 50** → Brand ships complete today. `map: 'vendor' | uniq` does NOT escape the cap; >50 would need `{% paginate … by 250 %}`. Tracked as forward-compat only.
- **Deploy:** watcher preflight PASS, target `role=main` confirmed; Admin-API readback **byte-for-byte MATCH** (SHA256 `0fff77a4fcf98b15`, 37,409 bytes; 0 `push`, 3 `concat: _one`, card-suppression present).
- **❌ Render NOT verified — DEFERRED.** CC's automated reads (curl / Section-Rendering-API / cart-cookie bypass) reported `l-shape-desks` Brand=5/Type=4/Room=2, but **Leo's real browser (incl. cart-cookie bypass) refuted this — facets still do NOT render for users.** The automated reads were FALSE POSITIVES. The deployed `push`→`concat` is byte-verified on the live theme but does not produce a rendered facet; root cause of the *non-render* is UNCONFIRMED. **Filter system DEFERRED (non-harmful, zero regression).**
- Pre-existing tag-hygiene noise (`type:desk` vs `type:desks`) noted for a later dedup pass.

**Live brand set (would populate the facet once it renders):** Global Furniture Group (200), Heartwood Manufacturing (80), OTG / Offices to Go (76), Intelligent Office Furniture (26), Office Star Products (18), Horizon (15), Richelieu (12), Safco/MityBilt/Teknion (11 ea), Fellowes (10), Deflecto (9) + a long tail down to single-product brands (ergoCentric, Humanscale, 3M, …) — **36 real brands**, dealer names suppressed.

**Next Phase 3 piece after the facet-render fix:** the Made-in-Canada filter + the `buy-canadian` Made-in-Canada collection (HELD handle, per the architecture call).

Report: `data/reports/brand-filter-bbi-suppression-2026-06-02.md`. Branch `feature/brand-filter-bbi-suppression-2026-06-02`.

---

## 🟢 Day 20 — 2026-06-02 — CATALOG: 225 vendor=BBI Re-sourcing (✅ EXECUTED — 97 + 5 = 225-resolution COMPLETE) · OPEN PR

**Unblocks the Brand filter.** The residual `vendor="Brant Business Interiors"` data-errors (BBI is a dealer, not a manufacturer) are the hard prerequisite before the Brand filter can ship — they'd otherwise surface a fake "Brant Business Interiors" brand chip. **Carry-forward** from the 2026-05-30 brand-recovery audit (PR #62: 183 corrected, 204 left UNKNOWN); since then `sku-prefix-lookup.yaml` gained deterministic decodes for **HDL/IOF/RIC/HZN/MTY**, which unblocks most of the tail. **✅ LIVE WRITES EXECUTED (hold lifted 2026-06-02): the staged 97 tier-1 corrections + the 5 ambiguous resolutions are written — the full 225-resolution is now COMPLETE. 0 theme files.** Report: `data/reports/bbi-resourcing-provenance-2026-06-02.md`. Worksheet: `data/reports/bbi-resourcing-2026-06-02.csv`. Branch `feature/catalog-bbi-resourcing-2026-06-02`.

**Scan (live count 221, scanned via Admin GraphQL `vendor:'Brant Business Interiors'`):**
- **CONFIDENT 115** (tier1 top-prefix **97** + tier2 sub-code/line/title **18**) · **AMBIGUOUS 87** · **SKIP 19** (services).
- Signal priority = SKU-prefix primary (SKU-PREFIX-PATTERNS-ARE-DETERMINISTIC), then structured-body / metafield / existing-tag / direct-name / model-code / sub-brand. Every signal traces to a locked reference (`sku-prefix-lookup.yaml` incl. `decoded_details`, `manufacturer-defaults.yaml`) or an unambiguous brand name in the product itself — **no free guesses**.

**Decisions (Leo, this session):**
- **HDL folded into Heartwood** — at execution time aligned to the **existing live canonical `Heartwood Manufacturing`** chip (see canonical-alignment note below), not a net-new `Heartwood` vendor.
- **Scope = tier-1 only (97).** The 18 tier-2 promotions (UN→IOF, INV/LEVELS→Heartwood, NLP→Global, Screenflex/Shoptech/Foundations/MVL title-signals) defer into the Steve/manual pile.
- **HOLD LIFTED (2026-06-02).** The 97 tier-1 writes executed live via `scripts/apply-bbi-resourcing-2026-06-02.py --tier1 --live`.

**🔑 Canonical-alignment correction (Phase-0 re-verify before write):** the live vendor list showed the staged decoder names would have **fragmented** existing chips — the store already standardized on `Heartwood Manufacturing` ×39 and `Horizon` ×5 (PR #62). Staged `Heartwood`/`Horizon Furniture` were therefore **aligned to the live canonical** before writing (Leo-approved): Heartwood 40 → **Heartwood Manufacturing** (`brand:heartwood-manufacturing`), Horizon 10 → **Horizon** (`brand:horizon`). IOF/Richelieu net-new; MityBilt already matched. The worksheet `recommended_vendor_correction` was updated to the canonical names (56 cells: 50 tier-1 + 6 tier-2 for consistency); `detected_manufacturer` left as the raw decode for audit.

**Tier-1 EXECUTED (97/97 PASS, all hardened readbacks MATCH, 0 guard skips):** Heartwood Manufacturing 40 (→79) · Intelligent Office Furniture 26 (new) · Richelieu 12 (new) · Horizon 10 (→15) · MityBilt 9 (→11) — each = vendor correction + matching `brand:*` tag (vendor + tag only; nothing else touched). **Hardened guards (5-write parity, added this session):** refuse any row where live vendor≠BBI, SKU drifted, or a BBI vendor/`brand:brant-*` tag would be written. **Independent Admin-API readback:** BBI 216→**119**; no `Heartwood`/`Horizon Furniture` fragment vendors created.

**🔑 Brand-filter readiness — remaining `vendor=BBI` = 119** (down from 221 at scan): **18** tier-2 deferred + **82** ambiguous (67 blank + 15 filled-but-held) + **19** services (SKIP — BBI-as-vendor is legitimate). The ~100 non-service residual still surfaces a "Brant Business Interiors" chip → **chip-suppression scope** until the Steve pass clears the tier-2 (18, cheapest — already sourced) + ambiguous (82). SCN (7) is locked-undecoded (pending supplier invoice). **Pre-existing fragment (out of scope):** 1 product still at vendor `HDL`/`brand:hdl` — fold into Heartwood Manufacturing in a follow-up.

**Scripts:** `scripts/scan-bbi-resourcing-2026-06-02.py` (scan), `scripts/apply-bbi-resourcing-2026-06-02.py` (writer — hardened guards, dry-run default; `--tier1 --live` executed), `scripts/gen-bbi-resourcing-report.py` (report-gen). Audit: `data/logs/bbi-resourcing-20260602-200920.log` + `data/backups/bbi-resrc-*-pre-*.json` (gitignored).

---

## 🟢 Day 20 (Phase 3) — 2026-06-02 — COLLECTIONS-ARCHITECTURE Phase 3: Nav Promotion + desks.json Repoint + Quiet Spaces Nav-Hop Fix (THEME WRITE) · OPEN PR

**First theme write since Surface B.** The Phase-3 payoff: surfaced the 16 promotable ranking collections, repointed the 2 dangling `desks.json` tiles (unblocks batch-2), and fixed the broken Quiet Spaces nav-hop. **Watcher-gated — SCHEMA-CRIT-1 PASS** (no `shopify theme dev` watcher on any theme, nothing to kill; role=main `186373570873` re-confirmed by Admin-API readback immediately pre-PUT). Report: `data/reports/collections-arch-phase3-nav-promotion-2026-06-02.md`. Branch `feature/collections-phase3-nav-promotion-desks-repoint-2026-06-02`.

**🔑 Phase-0 mechanism finding:** BBI's nav is **HARDCODED in theme files** (`snippets/bbi-nav.liquid` + `snippets/bbi-footer.liquid`, gated on via `theme.liquid`), **NOT** a Shopify linklist — so nav promotion = a CC theme write, no `navigation`/`write_content` token scope needed. The "Quiet Spaces nav item" was never in the header; it was card #08 of the 9-card "Browse by category" grid in `ds-lp-industries.liquid` + the shared `ds-browse-faq.liquid` (5 spoke pages), which 301-hopped to accessories.

**Decisions (Leo):** nav mechanism = **sub-collection tiles** (proven `desks.json` pattern; not a mega-menu rebuild); Quiet Spaces card = **repoint → Phone Booths** (`telephone-booths`); tile labels kept **keyword-anchored** (distinct ranking handles, not softened).

**Writes (10 files, all PUT=200, all hardened-readback MATCH — 7 JSON parsed-match + 3 Liquid byte-match):**
- **A — desks.json repoint:** `tile-height-adjustable` → `/collections/height-adjustable-tables`, `tile-computer` → `/collections/multi-person-workstations` (no more `-desks` sources).
- **B — 14 sub-collection tiles** (no image → graceful text placeholder): seating +4 (bariatric / folding-stacking / nesting / recliners), tables +4 (flip-top / bar-height / coffee / cafeteria), storage +3 (fireproof files / fireproof safes / pedestals-drawers), boardroom +1 (lecterns-podiums), panels +1 (desk-top-dividers), accessories +1 (Phone Booths → telephone-booths). Reception-desks already a desks tile; **gaming → footer** (off-ICP). All 16 promotable handles accounted for.
- **C — Quiet Spaces nav-hop fix:** card #08 → Phone Booths / `telephone-booths` in `ds-lp-industries.liquid` + `ds-browse-faq.liquid`.
- **D — footer:** +`Gaming Chairs → /collections/gaming` in `bbi-footer.liquid` Shop column.

**Verification:** theme-check = 0 new offenses (none of the 10 files in the offense list); live cache-busted render confirmed all new tiles + Phone Booths cards + desks repoint targets + footer Gaming link; no `quiet-spaces` leftover on the two card surfaces.

**Noted (NOT fixed — distinct ranking handles):** 4 near-dup tile-label pairs — Stacking&Training vs Folding&Stacking (seating), Training&Folding vs Training/Flip-Top (tables), Mobile Pedestals vs Pedestals&Drawers (storage), Podiums&AV vs Lecterns&Podiums (boardroom).

**Tracked follow-up:** IMAGE-GEN for the 14 image-less tiles (`bbi-coll-img-<parent>-tile-<key>-v*.jpg` convention).

**⚠️ CRITICAL SEQUENCING (PR + Steve list):** batch-2 desk-families CSV (`collections-phase2-batch2-deskfamilies-301s.csv`, 9 rows) stays gated — **only after this PR merges + theme is live** does Steve import it; **then** a batch-2 source-unpublish follow-up activates the 9 redirects (same dormant-redirect mechanic as batch-1). **Never import batch-2 before this repoint deploys.**

---

## 🟢 Day 20 — 2026-06-02 — COLLECTIONS-ARCHITECTURE: Publish-State Reconciliation (Admin-API publish toggles) · OPEN PR

**Activates the dormant redirect folds + rescues the rank-bleeding collections** surfaced by the Day-19 Phase-3-prep verification. **Admin-API publish toggles ONLY — 0 theme writes (no watcher gate), 0 redirect writes, 0 menu writes.** Dry-run → HALT → Leo approval → 5 live writes, per-item hardened readback. Report: `data/reports/collections-publish-state-reconciliation-2026-06-02.md`.

**🔑 Locked learning — Shopify URL redirects are DORMANT while the source collection is published.** A `/collections/<h>` redirect is inert until `<h>` is unpublished. So: **folding** a collection needs the redirect entry **AND** an unpublished source; **rescuing** a self-redirecting collection = **republish it** (redirect auto-dormant, serves its own 200, rank returns). This explained two stalled migrations (the dormant bc-m→boardroom 301; 4 rankers bleeding rank to a nav parent).

**Writes executed (5, all readback MATCH):**
- **A — RESCUE (publish=true):** `benching-desks` (#72), `coat-racks-accessories` (#34), `modesty-panels` (#36), `picnic-tables` (#48) — all now serve their own 200, rank reclaimable. *Thin (1–2 products) → repopulate + internal-link later.*
- **B — FOLD (publish=false):** `reception` → now 301 → `reception-desks-desks` (#23 canonical). Guard cleared: not a protected ranker, its 3 products ⊆ canonical's 9 (orphans nothing). Completes the Day-19 landing-refresh intent.
- **healthcare-seating** = NO-OP — was already unpublished + firing 301 → /pages/healthcare (done Day 19).

**Decisions:**
- **boardroom-conference-meeting → KEEP-CANONICAL** (Leo). bc-m ranks #22 (modular boardroom tables); nav `boardroom` has no captured rank → don't fold a ranker into a rank-less collection. Steve drops the bc-m→boardroom redirect (Steve list, no CC write).
- **keilhauer → HOLD.** Empty 200, no redirect yet; Steve's call (empty either way), not urgent.

**Carry-forward:**
- **✅ DONE — GATED batch-1 source-unpublish (33 handles):** Steve imported `collections-phase2-batch1-301s.csv`; the gated follow-up ran 2026-06-02 (see the **Day-20 follow-up** section directly below). 33 unpublished + firing; keilhauer held.
- **Steve list (in PR):** (1) ~~import batch-1 CSV → ping Leo~~ ✅ done (CSV imported, follow-up shipped); (2) delete `executive-desks→desks` redirect (unblocks exec-desks build); (3) delete `bc-m→boardroom` redirect (keep-canonical); (4) hygiene — delete the 4 now-dormant rescue redirects.
...
- **Phase 0 (read-only safety crux):** all **34 / 34** batch-1 redirects present in the live Admin-API map (import landed fully), all 34 live targets == CSV targets (faithful), **15 / 15** unique targets resolve 200 zero-chain, HARD GATE not tripped (0 missing redirects), **0 / 33** sources are ranking handles. Script `scripts/phase0-batch1-classify.py` → `data/reports/phase0-batch1-classify-2026-06-02.json`.
- **Phase 2 (executed):** **33 sources unpublished** — 14 Batch B (scaffold empties → nav parent) + 14 Batch C1 (type-*/room-* dupes → nav twin) + 5 Batch C2 (legacy dupe families → ranking `-storage` keeper). **33/33 hardened-readback MATCH** (REST `published_at=null` authoritative) and **33/33 storefront 301 → target already firing** (no edge-lag, no 404, no chain). Script `scripts/phase1-batch1-unpublish.py --live`; log `data/logs/batch1-unpublish-2026-06-02.json`; pre-state backups in `data/backups/batch1-unpub-*-2026-06-02.json`.
- **Already-firing:** 0 (no source was pre-unpublished this round).
- **keilhauer still HELD** — redirect imported + live (Batch D), source left **published** per Steve's pending decision; ranking handle #22, double-protected. Fires only on Steve's go.
...
**Preflight gate (hard stop) — PASSED:** no `shopify theme dev` watcher running (none bound to any theme); `186373570873` confirmed **role=main** via Admin-API readback (name "BBI Landing Dev" is historical, ignored). All 5 assets backed up to `data/backups/surface-b-theme-backup-20260601.json` before writing.

| Page | Edit | Surface | Verified live |
|---|---|---|---|
| **Design Services** (hub) | H1 → "Free office space planning — your to-scale CAD floor plan, no obligation." + eyebrow "Service · Office Space Planning" + intro "handles the office space planning". `office space planning` aligned; "free CAD floor plan / no obligation" hook + `Get My Free Layout` CTA kept. | `page.design-services.json` (template only) | ✅ fresh render |
| **Professional Services** (spoke) | H1 → "Law Firm Office Furniture — and Specification-Grade Furnishings for Every Ontario Practice" (exact-phrase lead, per Steve); subheading leads with exact "Law firm office furniture —"; **added 4th funnel tile → `/collections/reception-desks-desks`** (canonical) and grid → `repeat(4,1fr)`. Boardroom tile already → `/collections/boardroom` (canonical, unchanged). Exec-desks **HELD** (collection not built). No links to 301'd `/collections/reception` or `/collections/boardroom-conference-meeting`. | `page.professional-services.json` (template) + `ds-lp-professional-services.liquid` (section) | ✅ fresh render |
| **Healthcare** (spoke) | H1 → "Canadian Healthcare Furniture for Clinics & Clinical Environments"; subheading → "Brant Business Interiors is a **Canadian** healthcare furniture supplier … across **Ontario**". `healthcare furniture canada` aligned as **identity, not nationwide reach** (Steve's correction — page is Ontario-centric: `area_served: "Ontario, Canada"`, Ontario install crew). Private-clinic-first tone kept. | `page.healthcare.json` (template only) | ✅ fresh render |
| **R5 — sitewide `<title>`** | `layout/theme.liquid`: append short "Brant Business Interiors" (24ch) instead of 41-char `shop.name`, and only when `page_title \| size <= 33` so rendered titles stay ≤60. Suppression for Brant/OECM/shop.name title_tags kept. | `layout/theme.liquid` (sitewide) | ✅ broad sample |

**R5 verification (broad blast radius):** swept 16 PDPs + 6 collections + 5 pages with per-URL cache-busting. **0 titles >60.** Previously-overflowing PDPs fixed (`l-shape-desk` 78→34, `monitor-stand` 78→34, `2-drawer-legal` 80→36). Collections (42–56) + pages (48–60) **unchanged** — optimized title_tags + brand-suppression intact, nothing pre-existing broke.

**Verification discipline:** every PUT returned HTTP 200; Admin-API readback = 2 `.liquid` byte-identical + 3 JSON semantically identical (byte diff is Shopify's cosmetic unicode re-escape, content verified by parsed-value compare). Live render confirmed by distinct-string grep per page after busting the edge full-page cache (JSON-template edits don't auto-purge the edge cache; `.liquid` edits do). **Theme-check:** 2834 pre-existing offenses (baseline unchanged); my 5 files = 7 warnings / 0 errors — 3 JSON templates clean, `theme.liquid` warnings all pre-existing (none in the title block), `ds-lp-professional-services.liquid` +1 `HardcodedRoutes` from the reception tile matching the file's existing idiom (every tile hardcodes its `/collections/` path).

---

## 🟢 Day 19 — 2026-06-01 — POST-S6 CLOSE-OUT — GFG WARRANTY DEFAULT NULLED (#75 ✅ MERGED) + COLLECTIONS-AUDIT (Phase 0, read-only) + LANDING-REFRESH-PLAN & SURFACE A EXECUTION + SITE-HEALTH REGRESSION AUDIT

Day-19 close-out after Session 6. Three read-only analysis artifacts + one Admin-API execution surface (Surface A) + the one-line warranty-default fix landed. **PRs #72, #73, #74, #75 all MERGED to `main` (`798efee`).** No theme files touched in any of this work. Provenance artifacts committed alongside this build-state update: `data/reports/collections-audit.md`, `data/reports/site-health-regression.md`, `data/reports/landing-refresh-plan.md`, `data/redirects/landing-refresh-301s.csv`.
...
- **Every ranking collection is a nav-orphan.** All 25 ranking handles sit *outside* nav — BBI's organic authority lives in legacy 2024 custom collections the new nav doesn't link to.
- **~120 empty-but-200 collections** (91 legacy + ~29 from a 2026-05-07 scaffolding batch) — thin/indexable crawl bloat.
- **3 ranking-but-EMPTY collections at risk** (`keilhauer` #22, `book-displays-storage` #43, `laboratory-furniture` #97) → **repopulate, NEVER redirect** (redirect forfeits the rank).
- **Brand-filter data is ready, unused:** `vendor` populated on **435/656 (66%)** — Global Furniture Group alone = 200 — while `/collections/global-furniture` sits as an empty scaffold. Highest-leverage Step-5 move = vendor dedupe (43→~30) → wire vendor→brand collections (also rescues ranking-but-empty `keilhauer`).
- **4 decisions kicked to the architecture web-chat call** (see carry-forward below).

### LANDING-REFRESH-PLAN + SURFACE A execution (`data/reports/landing-refresh-plan.md` + `data/redirects/landing-refresh-301s.csv`)
The SEO **meta** layer was already done in #70; this workstream is the visible on-page **body copy** refresh (a theme write — H1/hero/FAQ on the 3 landing pages still don't contain the locked primary KWs) **plus** the collection cannibalization fix. The D1 **soundness-gate invalidated 3 approved premises** before any write (exec-desks unpublished + empty; healthcare-seating empty; boardroom canonical is a genuine 13-products-ranking vs 25-products-optimized trade-off).
**Surface A executed (Admin-API, all PUTs readback-verified):**
...
Read-only structural-SEO/AEO check of today's changed surfaces (53 S5+S6 PDPs + touched collections + 3 landing pages). **Bottom line: no structural regression introduced.** 53/53 PDPs clean — 200, indexable, in sitemap, valid `Product` + `BreadcrumbList` + sitewide `Organization`/`LocalBusiness`/`WebSite` JSON-LD, server-side spec blocks, `title_tag` ≤60, desc ≤160, consistent NAP, zero broken/redirect-source links. All 5 action items live in the cannibalization/redirect layer and **none were introduced today**:
- **R1 (fixed in CSV):** Row 4 was `healthcare-seating → healthcare`, but `/collections/healthcare` itself 301s → `business-furniture` (a chain) → **repointed to `/pages/healthcare`** in the shipped CSV.
- **R3:** reception pair is now an identical-meta twin until Row 1 imports — import promptly.
- **R5:** rendered `<title>` >60 on 38/53 PDPs — a **pre-existing sitewide theme title-suffix behaviour**, not a regression (theme untouched today); flagged as a later theme-scoped hygiene fix.

### 🟢 Day 19 close-out — OPERATIONAL LESSONS (3)
1. **READ-ONLY-SOUNDNESS-GATE-BEFORE-A-WRITE-SESSION.** The D1 gate caught 3 invalidated approved premises (exec-desks unpublished+empty, healthcare-seating empty, boardroom non-clean canonical) *before* any migration write. Confirm a collection is structurally sound — published + populated + right template — not just *ranking*, before scoping a redirect/consolidation.
2. **DEFAULTS-IN-REFERENCE-DATA-ARE-TRAPS (closes S6 Lesson 1).** The warranty over-stamp root cause was a hardcoded `Limited Lifetime` default in `manufacturer-defaults.yaml`; nulled in #75. Never leave a "convenient default" that auto-source agents will silently apply — prefer source-or-empty with an explicit null + note.
3. **REDIRECT-CSV-CHAIN-CHECK.** Verify every redirect *target* resolves 200 and doesn't itself redirect before shipping the CSV. Row 4 (`healthcare-seating → healthcare`) would have chained to `business-furniture`; repointed to `/pages/healthcare`.

### Block 4 running total — unchanged
**85 products enriched** (S1-6). No new enrichment in the close-out; this was analysis + meta/redirect surface + the warranty-default fix.
...
## 🟢 Day 19 — 2026-06-01 — PHASE-A-BLOCK-4-SESSION-6 — CATALOG ENRICHMENT (26 Global-family PDPs) + WARRANTY SOURCE-OR-EMPTY · PR #74 ✅ MERGED

**PHASE-A-BLOCK-4-SESSION-6 — 26 products enriched** (continuation of S5/PR #73). Same 13-field `specs.*` framework + body rewrite + SEO + tags + priority-keyword v1. Production Admin-API writes only — **zero theme files touched**. Branch `feature/phase-a-block-4-session-6-2026-06-01` off `main`. **Do not auto-merge — Leo reviews.**

### Batch composition (all Global Furniture Group family; GLB/GLO/MVL prefixes)
- Cluster-weighted per rank-tracking priority: **10 waiting-room/guest seating · 4 lounge seating · 4 tables · 1 exec-desk-held-out · the rest exec/task seating (descriptive)**. All 26 → `specs.manufacturer = "Global Furniture Group"`, sub-brand → `product_line`.
- **Reconciliation cleared a false alarm:** the live "126 enriched" Global count = 120 fully-enriched (incl. pre-Block-4 Pass 1/2) + 6 partial/stamped-only — NOT ~67 missed. The 74-no-manufacturer pool is the correct net-new target. (6 partial top-ups flagged for a later micro-batch.)
- **Held out:** #17 `l-shape-desk-with-36h-privacy-panel` (mixed Global panel + Heartwood Innovations desk bundle — manual handling). The only true exec desk in the pool; reception cluster had **0 net-new** (all reception desks already enriched S1-5).

### Results
- **Auto-source:** 10/26 success · 16/26 partial (series/dealer fallback) · 0 fail. Parallel 26-agent fan-out; **first run hit the Anthropic usage session limit after 7 agents** — resumed from cache after the 1pm reset (the 7 returned cached, 19 re-ran). Lesson: large fan-outs can exhaust the rolling session quota; `resumeFromRunId` makes recovery idempotent.
- **Body state:** all 26 REAL existing copy, **0 boilerplate-corruption, 0 pre-existing specs** — clean slate.
- **Agent-corrected cluster mis-assignments (no-forced-fit rule working):** Craft round/wedge/overtable are **modular lounge poufs/wedges, not conference tables**; `work-table-at` is an **OTG value training table**; `sidero-1` is a guest/stacking chair; `rambler` is a lounge ottoman. All re-tagged out of boardroom; **zero forced boardroom keywords landed.**
- **Honest keyword accounting:** 6 genuine `office chairs for waiting room` landers (verified against **rendered** copy, not landed-metadata — Session 5 false-positive lesson applied); 0 boardroom / 0 executive-desk (none were genuine tables/desks); 20 descriptive/zero-locked. Brand terms descriptive only.
- **Certs:** GREENGUARD never auto-applied; seating left `certifications=[]` + flagged where unconfirmable; ANSI/BIFMA applied only where sourced.

### Warranty SOURCE-OR-EMPTY — discipline held (0 over-stamps)
The S5 failure mode (agents defaulting "Limited Lifetime") was pre-empted: both controlling matrices were **inlined into the draft script** and the `manufacturer-defaults.yaml` lifetime default was explicitly neutralized in the agent rules. Result:
- **All 9 value/MVL items carry NO lifetime** (OTG matrix: seating 2yr, laminate non-moving 5yr). Caught + corrected at QA: **chevron** (Basics `BAO` value line) had a premium-frame lifetime leak → corrected to **2yr** per Leo; **DTS occasional tables** (`DTS1828P` twins) diverged (one lifetime/one empty) → both set to **5yr value non-moving** per Leo, dims resolved (overall 29"W×25"D×28"H, worksurface 28"W×18"D — legacy titles referenced different facets of one product).
- **GFG-premium seating** → sourced limited-lifetime-frame + 5yr upholstery/foam/mesh + 12yr mechanism (Global warranty page).
- QA gate (`scripts/b4s6-qa.py`): warranty↔body consistency 0 mismatches, stray-figure scan clean, title ≤60 / meta ≤160 enforced, 0 literal "BBI". **Final: 0 FAIL / 0 WARN.**

### Safety / verification
- Preflight `scripts/preflight-write-check.sh`: **PASS** immediately before writes (watcher clear; role=main `186373570873` verified). Admin-API **product** writes — independent of theme assets.
- **Writes 26/26 · hardened readback 26/26 MATCH** (seo + descriptionHtml + tags + vendor + productType + every specs metafield) · storefront curl 26/26 HTTP 200.
- **RRT spot-check (5 PDPs):** Product + BreadcrumbList JSON-LD valid; new content confirmed live (not stale edge) — chevron renders "2-year warranty" ×6 / **0 lifetime**, DTS renders "5-year non-moving" / **0 lifetime**, marche renders "office chairs for waiting room" ×3.
- **Theme check held** — `git status theme/` clean (0 theme files).
- Artifacts: 26 pre-write snapshots `data/backups/session-6-*-pre-*.json` (gitignored), write log `data/logs/session-6-*.log` (gitignored), review cards `data/reports/b4s6-review-cards.md`, payloads `data/reports/b4s6-payloads/`, scripts `scripts/b4s6-*.py` + draft workflow `scripts/b4s6-draft-workflow.js`.

### 🟢 Day 19 (2026-06-01) — OPERATIONAL LESSONS (2)

1. **AGENT-WARRANTY-DEFAULTS-NEED-SOURCE-VERIFICATION (CONFIRMED — promoted from S5 candidate).** Auto-source agents over-assert "Limited Lifetime" when a default exists in reference data. The fix that worked in S6: (a) **inline both controlling warranty matrices into the draft script** as literals, (b) **explicitly neutralize the `manufacturer-defaults.yaml` lifetime default** in the agent rules ("IGNORE that default; source-or-empty"), (c) route per-prefix warranty basis (OTG-VALUE vs GFG-PREMIUM), (d) **QA gate** asserting warranty↔body consistency + no-lifetime-on-value + stray-figure scan. Result: 0 over-stamps across 26 (vs 14 metafields + 17 bodies in S5). Note `manufacturer-defaults.yaml` STILL hardcodes `global_furniture_group: warranty: Limited Lifetime Warranty` — recommend changing it to `null` with a "SOURCE PER PRODUCT — NEVER DEFAULT" note to remove the trap at source.
2. **LARGE-FAN-OUT-CAN-EXHAUST-SESSION-QUOTA — resume is idempotent.** A 26-agent parallel draft fan-out hit the Anthropic rolling usage session limit after only 7 agents completed; the other 19 errored instantly with "session limit · resets 1pm". `Workflow({resumeFromRunId})` after the reset returned the 7 completed agents from cache and re-ran only the 19 — zero rework. For big fan-outs, expect possible mid-run quota exhaustion and plan to resume rather than restart.

### Block 4 running total
**85 products enriched** across Sessions 1-6 (S1=5, S2=8, S4=19, S5=27, **S6=26**; S3 = 183 vendor corrections, not enrichment).

**Next:** PR #74 ✅ MERGED (Day-19 close-out). Remaining Global pool ~48 (mostly accessories/storage/filing + 6 partial top-ups + the held-out mixed-mfr bundle). Then the LANDING-PAGE-THEME-CONTENT-REFRESH (highest acquisition impact per rank-tracking).

---

## 🟢 Day 19 — 2026-06-01 — PHASE-A-BLOCK-4-SESSION-5 — CATALOG ENRICHMENT (27 Global-family PDPs) + PRIORITY-KW v1 APPLIED + WARRANTY RE-SOURCE · PR #73 ✅ MERGED
...
## 🟢 Day 18 — 2026-05-31 — SEO RECON → KEYWORD MAP → PRIORITY-PAGE REFRESH + HEADER TAGLINE · **5 PRs (#66–#70)** ✅ SHIPPED

**Day 18 shipped 5 PRs (#66–#70)** — an SEO-strategy chain plus one header polish. All work is docs / SEO-meta / data-reference except #67 (a single shared theme snippet); no risky bulk theme writes. In dependency order: overnight **competitor keyword recon** (#66) → **Office Central header tagline** v1→v3 (#67) → Block 4 Session 3 **SKU-prefix decode** wrap-up (#68) → **ICP-KEYWORD-WALKTHROUGH session 1** producing the priority-keyword map + machine-readable lock file (#69) → **Priority Page Content Refresh** applying SEO meta to 7 live surfaces + the new **Phase 0 architecture-discovery gate** (#70). *(UTC merge timestamps for #68–#70 cross into 2026-06-01, but all are Day-18 evening work in Leo's Eastern frame.)*

### 🟢 Day 18 — COMPETITOR-KEYWORD-RECON-1 ✅ SHIPPED 2026-05-31 · PR #66

- **COMPETITOR-KEYWORD-RECON-1 ✅ SHIPPED 2026-05-31** (merge `d0a60b8`; PR [#66](https://github.com/leokatz97/officecentral/pull/66)). Overnight **read-only** DataForSEO competitor + BBI baseline keyword reconnaissance — **no production writes.** Coverage: **12 competitors** (10 Ontario-priority + QC + BC); the original `shaw-furniture.com` / `furniture-options.ca` seeds returned **zero Canada rankings** and were dropped, expanded with verified Ontario Tier-1 dealers (atwork.ca, sourceofficefurniture.ca, mapofficefurniture.com, officestock.com, barrysofficefurniture.com, grandandtoy.com, theofficeshop.ca, poi.ca, vaughanofficefurniture.com, newmarketofficefurniture.com) via DataForSEO competitor analysis + Leo's Maps list. **2,428** competitor keywords aggregated (deduped, search volume ≥ 10). **BBI baseline: 70 ranked keywords; Office Central: 0** (parent site, minimal SEO footprint). Top-30 highest-volume keywords enriched with PAA / featured-snippet / AI-Overview / local-pack flags; 54 geo keywords checked for local pack. **Classification:** 9 intent categories × 5 BBI verticals × 5 outcome paths × 3 tiers × 12 site-surface recommendations (0 keywords left unresolved). Committed: `data/reports/keyword-research/SUMMARY-2026-05-30.md` + analysis report set + 8 `scripts/recon_*.py` / `scripts/dfs_client.py`. Bulk per-domain CSVs, raw JSON, and `.mcp.json` gitignored. Driven via the **DataForSEO REST API directly** (credentials from env). This is the recon that feeds the #69 keyword map. *(See memory `project_keyword_recon_2026`.)*

### 🟢 Day 18 — HEADER-OFFICE-CENTRAL-TAGLINE (v1→v2→v3) ✅ SHIPPED · PR #67

- **HEADER-OFFICE-CENTRAL-TAGLINE ✅ SHIPPED 2026-05-31** (branch `feature/header-office-central-tagline-v2-2026-05-31`, off `main` @ `d0a60b8` = PR #66 tip). Adds a small supporting tagline **"A division of Office Central"** below the header wordmark, sitewide, for Office Central inbound brand recognition. Single file: `theme/snippets/bbi-nav.liquid` (shared header snippet, renders across **30 sections**). Preflight `scripts/preflight-write-check.sh`: RESULT: PASS (watcher clear; role=main `186373570873` verified). Theme check baseline `2833/165` held throughout. Commit `e08da45`; PR [#67](https://github.com/leokatz97/officecentral/pull/67).
  - **THREE-ITERATION EVOLUTION (all in one session, approval-gated writes to LIVE):**
    1. **v1 — ROLLED BACK.** Kept the header bar at 72px and let the brand-block (logo + tagline) push the logo *upward* to make room for the tagline below → top-heavy, awkward. Rolled back to byte-identical pre-state (confirmed IDENTICAL SHA at session start). Stale empty branch `feature/header-office-central-tagline-2026-05-27` (local-only, never pushed) deleted via `git branch -D`.
    2. **v2 — bar bump.** Grew `.bbi-header__inner` 72px → **80px** so the brand-block centers as a unit and the logo position no longer shifts (≤3-4px, reads intentional). Also bumped `.bbi-nav__item` 72px → 80px so the active-nav underline (pinned `bottom:0`) stays **flush with the header's bottom border** instead of floating ~4px up inside the taller bar — a consequence of the bar growth, same logical change. Tagline shipped at 12px. **Audit finding:** the `@media(max-width:1023px)` block does NOT override header height — bar is 72px (now 80px) **sitewide**, no separate mobile value.
    3. **v3 — measured final.** Real-device feedback: v2 tagline too large + not indented. **Measured `theme/assets/bbi-logo-v2.png`** (1360×400 natural, rendered `height:44`, scale 9.091): "B" of "Brant" left edge = 56 natural px → **6.16 rendered px** → `padding-left:6px` (aligns tagline start under the "B"); "BUSINESS INTERIORS" cap-height ≈ 69-70 natural px → **~7.6px rendered**. Surfaced the **font-size ≠ cap-height** nuance (Inter cap-height ≈ 0.71×font-size) → Leo chose **11px** (Option B, exact cap-match: 11px → ~7.8px caps). Added explicit `font-style:normal` (never italic — older-buyer mobile readability). Removed the now-redundant v2 mobile `font-size:11px` override (logo is 44px on mobile too → indent + size uniform across breakpoints, no scaling).
  - **FINAL SPEC SHIPPED:** bar 80px · nav-item 80px · tagline `font-size:11px` / `padding-left:6px` / `#666666` / `font-weight:400` / `font-style:normal` / `white-space:nowrap` / `margin-top:3px`. Brand-block wrapper stays flush to the logo bounding box; indent applied to the `.bbi-header__tagline` span only (per Leo's instruction).
  - **VERIFICATION (per CURL-VS-ADMIN-API methodology):** each LIVE PUT gated on **Admin-API asset readback byte-match** (`…/themes/186373570873/assets.json?asset[key]=snippets/bbi-nav.liquid` → `preflight-byte-compare.py` → IDENTICAL). Note: **read-after-write replica lag** observed on every push — first refetch returned the prior asset version (SEMANTIC_MISMATCH on the just-changed line), IDENTICAL on retry ~5s later. Theme check `2833/165` held after each write. Render-checked /, /collections/desks, a PDP, /pages/about, /pages/oecm — confirmed tagline + brand markup live (edge full-page cache lagged on some surfaces, as documented; Admin-API readback is the authoritative gate). Real-device phone check **passed** ("header-tagline v3 good").
  - **LIVE `updated_at` trail:** `19:23:00` (post-v1-rollback baseline) → `20:39:23` (v2 brand+bar) → `20:42:12` (v2 nav-item) → `20:56:04` (v3 size+indent+style).
  - **NOTE for future operators:** the v2 branch name carries through to the v3 ship (work evolved past v2 within the session; branch created at session start). The logo lockup is the **Brant Basics** wordmark (Brant/BASICS left, BUSINESS INTERIORS right) — the "B of Brant" indent reference is the top-left black "Brant", not the BASICS row.

### 🟢 Day 18 — SKU-PREFIX-DECODE-V2 ✅ SHIPPED 2026-06-01 (Block 4 Session 3 wrap-up) · PR #68

- **SKU-PREFIX-DECODE-V2 ✅ SHIPPED** (merge `173a135`; PR [#68](https://github.com/leokatz97/officecentral/pull/68)). Wrap-up of the Block 4 Session 3 SKU-prefix decode — updates `data/reference/sku-prefix-lookup.yaml`, **no catalog writes.** **4 prefixes decoded** with catalog + source evidence: **HDL → Heartwood Distributors Ltd.** (distributor, multi-OEM; same family as existing `HTW`), **IOF → Intelligent Office Furniture** (Canadian-made manufacturer; vendor field literally `IOF`), **RIC → Richelieu / Richelieu Ergo** (very high — `RIC-5008479C3` is an exact Richelieu JAX part-number match), **HZN → Horizon Furniture** (seating + accessories distributor; Shoptech series). **ICN → Icon Office** added from institutional knowledge but ⚠️ **not present** in `products-export-2026-04-21.csv` (0 ICN SKUs) — flagged in-file to confirm against the live catalog before any vendor backfill. **SCN deferred** (steel lockers + outdoor site furnishings; needs Steve confirmation via supplier invoice — closest matches Anthony Steel Mfg. / Canaan Site Furnishings). Structure change: added a `decoded_details:` block (supplier / type / sub-codes / source / note) alongside the flat `sku_prefix_lookup:` map so the deterministic matcher stays intact; `undecoded:` converted list→map to carry SCN's resolution note. Vendor-backfill CSV / push deferred to a later session. *(See memory `project_enrichment_accelerator_limits`; builds on Day 17 evening lesson SKU-PREFIX-PATTERNS-ARE-DETERMINISTIC.)*

### 🟢 Day 18 — ICP-KEYWORD-WALKTHROUGH session 1 ✅ SHIPPED 2026-06-01 · PR #69

- **ICP-KEYWORD-WALKTHROUGH session 1 ✅ SHIPPED** (merge `adbdc54`; PR [#69](https://github.com/leokatz97/officecentral/pull/69)). Turns the #66 recon into a **per-page keyword map** + a **machine-readable partial-lock file** that Session 5+ Phase 0 can read. Docs-only. Two files: `docs/strategy/bbi-keyword-map-2026-05-31.md` (human-readable, 4 clusters + decisions + lessons) and `data/reference/priority-keywords.yaml` **v1, partial-lock** (validated YAML, structured page→keyword assignments). **4 clusters locked:** (1) **Design Services** = hub, primary KW `office space planning`; (2) **Professional Services** = spoke, `law firm office furniture`; (3) **Healthcare** = spoke (A/B straddle), `healthcare furniture canada`; (4) **Product Collections ×4** = funnel targets (reception / executive desk / boardroom / waiting-room). **Key decisions:** dental routed to **Healthcare** (not Professional Services) on clinical-environment + medical-SERP adjacency, with the reception collection as the shared touchpoint; **hub-and-spoke architecture validated** across all 4 clusters (service pages = topic hubs, industry pages = conversion spokes, collection pages = fat-volume funnel targets). Deferred to walkthrough session 2 (this week): Education + library (ICP A), Eastern-Ontario geo, brand-dealer pages, ergonomic seating. *(See memory `project_seo_strategy_2026`; this is the source-of-truth input to #70.)*

### 🟢 Day 18 — PRIORITY-PAGE-CONTENT-REFRESH + Phase 0 architecture-discovery gate ✅ SHIPPED 2026-06-01 · PR #70

- **PRIORITY-PAGE-CONTENT-REFRESH ✅ SHIPPED** (PR [#70](https://github.com/leokatz97/officecentral/pull/70) merge `4561206`; branch commits `8035589` + `d6500a2`). Applies the #69 locked keywords to **7 live surfaces** as **SEO-meta-only** (re-scoped by Leo at Phase 2 — **zero theme files touched**; on-page H1/intro/FAQ/CTA/link work on the 3 landing pages deferred to a follow-up theme-edit session). **Surfaces (1 hub + 2 spokes + 4 collections):** `/pages/design-services`, `/pages/professional-services`, `/pages/healthcare`, `/collections/reception`, `/collections/office-suites-desks`, `/collections/boardroom`, `/collections/reception-side-guest-chairs`. **SEO title + meta rewritten 7/7** (titles ≤60ch, metas ≤160ch, full "Brant Business Interiors" — never "BBI", healthcare kept private-clinic-first). **Collection description body** (FAQ + FAQPage JSON-LD 4-Q&A + 2 internal links → `/pages/healthcare`, `/collections/reception`) added to **1** surface — `reception-side-guest-chairs`, the only funnel collection on the default template that renders `{{ collection.description }}`. **Fact-check correction:** an unverifiable "2.5–3 seats per exam room" ratio was softened to an operational heuristic — no unverifiable claim shipped. **Safety:** theme check **2,833/165 held identical** (0 theme files), Admin-API writes 7/7 + readback 7/7 MATCH, cache-busted storefront render 7/7 confirmed, 7 pre-write snapshots + log gitignored. **YAML-slug-vs-reality drift caught:** 4 locked slugs didn't exist / were empty and were remapped to nav-canonical product-bearing handles. **Slug-change flag:** `/pages/design-services` → `/pages/office-space-planning` recommended but **not executed** (needs 301 redirect + `write_content` scope — current gap; separate session). **Phase 0 architecture-discovery gate** (`d6500a2`): added a content/page-edit gate to `BBI-Session-Kickoff/01-safety-preflight.md` + keyword-map refs — future page-edit sessions must discover the content model before scoping (see operational lesson below). Audit record: [`docs/reviews/priority-refresh-2026-05-31.md`](../../docs/reviews/priority-refresh-2026-05-31.md). **Next:** follow-up "Landing Page Theme Content Refresh" session completes the hub-and-spoke internal linking; then 7–14 day re-crawl + DataForSEO rank tracking on `priority-keywords.yaml` v1.

### 🟢 Day 18 (2026-05-31) — OPERATIONAL LESSON (1)

1. **SHOPIFY-CONTENT-MODEL-NOT-UNIFORM** — BBI's Shopify content lives in **four different places** and is **not uniform across page types**: (a) **CMS `body`** (the page/collection rich-text field, Admin-editable); (b) **theme Liquid sections** (`ds-lp-*.liquid` etc., driven by `section.settings` — code edits only); (c) **template JSON** (`page.*.json` / `collection.json` wiring sections to settings); (d) **metafields** (theme-read-only). The 3 priority landing pages (#70) have an **empty CMS `body`** — all visible H1/intro/FAQ/CTA/link content is hardcoded in theme Liquid, so only the SEO `title`/`meta` (`global.title_tag` / `global.description_tag`) are editable without a theme edit. Collection description rendering is **template-dependent**: only the default `ds-collection-base` template renders `{{ collection.description }}`; `ds-cs-base` and `ds-cc-base` do not — so a description-body write surfaces on default-template collections only (3 of 4 funnel collections were SEO-meta-only for this reason). **Rule:** every future content / page-edit session MUST begin with an **architecture-discovery pass** — for each target surface, determine which of the four layers holds the editable content and which template renders it — *before* scoping the work. A SEO-meta task and an on-page-content task have different risk profiles, file targets, and preflight requirements; conflating them produces the "deferred to theme session" rework seen in #70. This gate is now encoded as **Phase 0** in `01-safety-preflight.md` (added in PR #70 / `d6500a2`). *(See memory `feedback_shopify_content_model_not_uniform`.)*
...
**North Star — UPDATED 2026-06-01:** the original goal — *ship the first cornerstone blog post live* — is ✅ **ACHIEVED** (Cornerstone Post 1 published LIVE 2026-05-23 at `/blogs/news/oecm-ontario-school-boards-office-furniture`, OECM/education vertical). **The North Star is now ongoing organic traffic:** keep shipping cornerstone content, build out the structural SEO foundation, and stand up measurement. Three phases below. _(This is the active execution plan; the Tier 1 / Tier 2 / Tier 2B inventory below remains the canonical per-item detail reference.)_
...
**Phase B — First blog publish — ✅ DONE (Cornerstone Post 1 LIVE 2026-05-23)**

- ~~Cornerstone Post 1 editorial pass~~ ✅ **LIVE** at `/blogs/news/oecm-ontario-school-boards-office-furniture` (published 2026-05-23, OECM/education vertical, article ID `689003888953`)
- ~~Publish via Admin API~~ ✅ done
- **Remaining Post-1 follow-ups (non-blocking):**
  - **Verify `Article` (BlogPosting) + `FAQPage` JSON-LD** actually emit on the live BBI `ds-article.liquid` render (RRT the published URL + validate)
  - **Wire inbound internal links *to* the post** from `/pages/oecm`, `/pages/education`, and related collections — it currently links *out* but has few links pointing *in*
...
- **Content production (primary mode after Phase B):**
  - **📋 CONTENT ROADMAP — `data/reports/content-roadmap-2026-06-03.md` (2026-06-03, docs-only, OPEN PR).** Keyword-grounded pillar/cluster plan reconciling PR #66 recon (2,428 competitor kw + 460 net-new seed whitespace) with Leo's 80 Reddit-mined topics. **Priority CONFIRMED P1>P2>P3>P4**, reframed by data: **P1 Design/space-planning** = biggest *content* whitespace (competitors absent: 5 design_service kw total; blog/service ≈0% of competitor traffic) + design-consultation conversion hub → production focus; **P2 OECM/public-sector** = uncontested moat (≈0 volume, cornerstone cadence) anchored by Post 1; **P3 Ergonomics/wellness/durability** = highest raw AEO/PAA demand (`ergonomics of chair` 14,800 + dense PAA vein) → its FAQ hub runs EARLY despite #3 rank, **targeting the winnable informational/question/PAA vein only — NOT high-KD commercial head terms ("ergonomic office chair"/"standing desk"), which manufacturers + review sites own and a regional dealer can't win; P1 stays the lead production focus**; **P4 Buying-in-Ontario** = net-new Eastern-Ontario geo + buy-Canadian, ties to geo landing pages. ~50 clusters mapped, ~25 dropped (home-office/WFH, thin/transactional, no-demand borderline). AEO: Article schema on all + FAQPage on ~30 question clusters. Sequence: Post 1 (done) → **P1 pillar** → **P3 AEO FAQ hub** → P1 clusters → P4 geo/buy-Canadian → P3 fills → P2 cornerstones (2 & 3). Pillar↔cluster↔commercial-page linking = the D3 pass, **HELD until Tier 1 URLs final**.
...
**Pending now:**
- **✅ DONE — Import landing-refresh 301 CSV (Day 19)** — Steve imported `data/redirects/landing-refresh-301s.csv` (4 rows) via Shopify Admin → Navigation → URL Redirects → Import; **redirects are now live.** Reception row = low risk (dupe→canonical); **boardroom row = higher risk** (redirected the previously-ranking `boardroom-conference-meeting` → optimized `boardroom`) — watch the next weekly DataForSEO rank snapshot, recoverable by deleting the redirect. Exec-desks row is a temp no-op (already live); delete it before the exec-desks build session gives that handle a real product home.
