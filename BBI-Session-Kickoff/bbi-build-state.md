...
**Last updated:** 2026-05-11 (Wave H — May 10-11 stabilization wave + PE Pass 1 triage CSV shipped; PROMPT-4 restructured to triage-first 3-pass workflow)
...
| SPEC-JSON-LD | `additionalProperty` in Product JSON-LD | ✅ | commit pending · `theme/snippets/bbi-product-jsonld.liquid` on dev `186373570873` | Added `additionalProperty` array to Product schema.org JSON-LD. Reads all 12 `specs.*` metafields (manufacturer, product_line, model_codes, dimensions, weight, weight_capacity, materials, finishes_available, certifications, warranty, country_of_manufacture). Conditionally renders each as `PropertyValue` — products with no specs get no array. Template change only — benefits all 257 products (Hero 100 + enrichment 157) as their metafields are populated. Pushed to dev theme. **Live canary test pending (SPEC-CANARY below).** |
| SPEC-HERO-PUSH | Hero 100 spec gap-fill + metafield push | 🟡 | `data/specs.json` (100 products) + `data/specs/*.json` (100 individual files) + `scripts/push-pe2-specs.py` | Hero 100 specs sourced by `lookup-specs.py`. Last push: 2026-05-08 (75 products, 174 fields). **Coverage gaps:** manufacturer 66%, model_codes 48%, weight 27%, weight_capacity 14%, warranty 28%, country_of_manufacture 47%. Push script exists and works. Re-run after SPEC-CANARY validates the JSON-LD template on live. Command: `python3 scripts/push-pe2-specs.py --live` |
| PE-PASS-3 | Push enrichment to Shopify (descriptions + specs + vendor) | 🚧 | `scripts/push-pe3-enrichment.py` written | **Blocked on enrichment sessions completing.** Script built: reads `data/reports/pe-pass2-output.json` (enrichment 157) or `data/specs.json` (Hero 100 via `--hero` flag), diffs against current Shopify state, writes `body_html` + 12 `specs.*` metafields + optional vendor override. Dry-run default. Two modes: `python3 scripts/push-pe3-enrichment.py --live` (enrichment 157) and `python3 scripts/push-pe3-enrichment.py --hero --live` (Hero 100 spec re-push). After enrichment sessions done: run both modes, then COLLECTION-CLEANUP-1. |
| SPEC-CANARY | Live canary test — one product with full spec data | ⬜ | TBD | Push `bbi-product-jsonld.liquid` (updated) to **live** theme for validation only. Then run `push-pe3-enrichment.py --handle=adapt-high-back-synchro-tilter-mvl11724 --live` to write full spec set for the Adapt chair. Validate: (a) spec table renders on live PDP, (b) JSON-LD `additionalProperty` shows in view-source, (c) Google Rich Results Test passes for that URL. After green: proceed with SPEC-HERO-PUSH + PE-PASS-3. 🔔 **Requires pushing snippet to live theme — confirm before running.** |
