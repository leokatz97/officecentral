...
**Pending now:**
- **🆕 Import landing-refresh 301 CSV (Day 19)** — `data/redirects/landing-refresh-301s.csv` (4 rows; strip the comment lines first) via Shopify Admin → Navigation → URL Redirects → Import. Reception row = low risk (dupe→canonical); **boardroom row = higher risk** (redirects the currently-ranking `boardroom-conference-meeting` → optimized `boardroom`) — watch the next weekly DataForSEO rank snapshot, recoverable by deleting the redirect. Exec-desks row is a temp no-op (already live); delete it before the exec-desks build session gives that handle a real product home.
