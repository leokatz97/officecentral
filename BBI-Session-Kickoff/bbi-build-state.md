...
## 🟢 Day 20 — 2026-06-02 — CATALOG: 225 vendor=BBI Re-sourcing (READ-ONLY scan + dry-run, HELD) · OPEN PR

**Unblocks the Brand filter.** The residual `vendor="Brant Business Interiors"` data-errors (BBI is a dealer, not a manufacturer) are the hard prerequisite before the Brand filter can ship — they'd otherwise surface a fake "Brant Business Interiors" brand chip. **Carry-forward** from the 2026-05-30 brand-recovery audit (PR #62: 183 corrected, 204 left UNKNOWN); since then `sku-prefix-lookup.yaml` gained deterministic decodes for **HDL/IOF/RIC/HZN/MTY**, which unblocks most of the tail. **READ-ONLY this session — 0 Shopify writes (HELD per Leo); 0 theme files.** Report: `data/reports/bbi-resourcing-provenance-2026-06-02.md`. Worksheet: `data/reports/bbi-resourcing-2026-06-02.csv`. Branch `feature/catalog-bbi-resourcing-2026-06-02`.
...
- **HDL folded into `Heartwood`** (single clean `brand:heartwood` chip; same brand family as HTW; avoids a distributor-named second chip). HDL/HZN/RIC are themselves distributors — same dealer-vs-OEM nuance as BBI — but HZN/RIC kept as their own vendor per the locked YAML; only HDL folded.
...
- **HOLD — report only.** No live writes; `scripts/apply-bbi-resourcing-2026-06-02.py --tier1 --live` is staged (dry-run verified) for a future go.

**Tier-1 write-plan (staged, 97):** Heartwood 40 · Intelligent Office Furniture 26 · Richelieu 12 · Horizon Furniture 10 · MityBilt 9 — each = vendor correction + matching `brand:*` tag (Phase-1 pattern: vendor + tag only, snapshot + per-item hardened readback, BBI-as-write-target guard, HALT on any mismatch).

**🔑 Brand-filter readiness:** writing the 97 unblocks the filter *for those products*, but **M is not small** — of the 106 untouched (87 ambiguous + 19 skip), **~59 are published-real products** (52 priced + 7 $0-quote) that would still render a "Brant Business Interiors" chip. **Recommendation:** either suppress the BBI chip in the filter UI until a Steve pass, or run the Steve pass on the tier-2 (18, cheapest — already sourced) + ambiguous (87) first. SCN (7) is locked-undecoded (lockers/site-furnishings, pending supplier invoice).

**Scripts (all read-only or dry-run-default):** `scripts/scan-bbi-resourcing-2026-06-02.py` (scan), `scripts/apply-bbi-resourcing-2026-06-02.py` (writer, dry-run default), `scripts/gen-bbi-resourcing-report.py` (report-gen).
