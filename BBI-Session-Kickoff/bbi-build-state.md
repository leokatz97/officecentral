...
2. **Blog #2 — P1 design/space-planning cornerstone** (Leo) — lead production focus per the locked roadmap; proven model from Post 1. **🆕 PUBLISH-PREPPED + SOURCED** (`data/content-drafts/01-office-space-planning-pillar.md`, PR `feature/post1-publish-prep-2026-06-03`, 2026-06-03). Every stat fact-checked + cited inline (CCOHS/Govt-of-Manitoba 6.7 m²/72 ft² workstation — *attribution corrected: CCOHS cites Manitoba, not its own standard*; Aquila 150–250 ft²/person; Archie 20–25 ft²/seat); all ⚠️ flags resolved (founding-1964 + Peterborough HQ confirmed vs live Post 1; "four basic layouts" softened — no single authoritative list). **PUBLISH PACK appended** (final title<60 + meta + `faq.items` 6 pairs *byte-verified verbatim* vs visible Common Questions + schema note + Steve-image flags + D3 link spec HELD + 10-step publish checklist mirroring Post 1: `POST /blogs/108557861177/articles.json` → `faq.items` metafield → Steve featured image → validate). **Remaining: Leo's voice pass → re-sync `faq.items` → publish.** 6 Decisions-for-Leo parked in-draft. *(Article/BlogPosting schema is template-level in `ds-article.liquid` → no action.)*
