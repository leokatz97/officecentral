...
**Carry-forward:**
- **GATED — batch-1 source-unpublish (33 handles):** runs only AFTER Steve imports `collections-phase2-batch1-301s.csv` (unpublishing before redirect exists = 404). Even post-import the dormant mechanic requires unpublishing each source for the fold to fire. List + trigger in the report (section D).
- **Steve list (in PR):** (1) import batch-1 CSV → ping Leo; (2) delete `executive-desks→desks` redirect (unblocks exec-desks build); (3) delete `bc-m→boardroom` redirect (keep-canonical); (4) hygiene — delete the 4 now-dormant rescue redirects.
