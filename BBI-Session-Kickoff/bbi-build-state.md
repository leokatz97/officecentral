...
## 🟡 Day 20 — 2026-06-02 — Phase 3 Brand filter: BBI suppression shipped · ⚠️ facet-render bug discovered · OPEN PR
...
**⚠️ DISCOVERY — the facet does not render (supersedes the "50-cap" follow-up).** Live-page verification (cache-busted full render **and** Section Rendering API `?section_id=ds-cs-base`, across `l-shape-desks`, `all-desks`, `desks`, `all-seating`, `executive-desks`, `seating`) shows the filter sidebar renders **only the Price filter** — **Brand, Type, and Room facets render zero chips on every collection, smart or manual.** Cards render fine (they're inside the `{% paginate %}` block). The facet-source loops fail: `collection.all_tags` (line 345) and the **unpaginated** `for product in collection.products` (line 352) **return empty** in the rendered section context — the classic Shopify behavior where `collection.products` referenced outside a `{% paginate %}` block yields nothing. The underlying data is valid (Admin-API confirms products carry `type:desks` / `room:private-office` and real vendors). So:
- ✅ Suppression logic correct, deployed, asset-verified, **forward-compatible** — when the render is fixed, BBI is already suppressed.
- ❌ **Brand filter is NOT live** and **must not be marked complete.** It cannot be functionally verified ("facet shows real brands / BBI absent") because no facet renders.
- 🔧 **Remaining Phase 3 piece (escalated):** fix the unpaginated vendor/tag enumeration so the Brand/Type/Room facets render across the whole collection — its own scoped, non-trivial Liquid task (likely: build the lists inside a `{% paginate %}` pass or via a working `all_*` source). This **replaces** the earlier "facet incomplete on >50-product collections (first-50 only)" framing — the loop isn't capping at 50, it returns 0.
