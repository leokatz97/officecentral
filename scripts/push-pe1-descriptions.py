#!/usr/bin/env python3
"""
PE-1 push: write product `descriptionHtml` from
data/reports/pe1-hero100-descriptions.csv into Shopify productUpdate.

Usage:
  python3 scripts/push-pe1-descriptions.py             # DRY RUN
  python3 scripts/push-pe1-descriptions.py --live
  python3 scripts/push-pe1-descriptions.py --limit=N
"""
...
CSV_PATH = ROOT / 'data/reports/pe1-hero100-descriptions.csv'
...
def fetch_current(handles):
    ...
    while pending:
        ...
        aliases = '\n'.join(f'  p{i}: productByHandle(handle: "{h}") {{ id handle descriptionHtml }}'
                            for i, h in enumerate(chunk))
        d = gql('{\n' + aliases + '\n}')
        for i, h in enumerate(chunk):
            n = d.get(f'p{i}')
            out[h] = {'id': n['id'], 'desc': n.get('descriptionHtml') or ''} if n else None
        time.sleep(0.5)
    return out

def update_desc(pid, html):
...
def main():
    ...
    print('Fetching current Shopify descriptionHtml...')
    current = fetch_current([r['handle'] for r in rows])

    BACKUP_DIR.mkdir(exist_ok=True)
    ts = datetime.now().strftime('%Y%m%d-%H%M%S')
    backup_path = BACKUP_DIR / f'pe1-descs-pre-push-{ts}.csv'
    with open(backup_path, 'w', newline='') as f:
        w = csv.writer(f); w.writerow(['handle', 'product_id', 'descriptionHtml'])
        for r in rows:
            c = current.get(r['handle'])
            w.writerow([r['handle'], c['id'] if c else '', c['desc'] if c else ''])
    ...
    for r in rows:
        ...
        new_html = r['draft_body_html']
        if c['desc'].strip() == new_html.strip():
            plan.append({'handle': h, 'status': 'UNCHANGED'}); continue
        plan.append({'handle': h, 'status': 'WILL_UPDATE',
                     'product_id': c['id'], 'old_desc': c['desc'], 'new_desc': new_html})
    ...
    if not live:
        ...
        for p in will[:3]:
            print(f'\n  {p["handle"]}')
            print(f'    OLD (first 200c): {p["old_desc"][:200]}')
            print(f'    NEW (first 200c): {p["new_desc"][:200]}')
    ...
    for i, p in enumerate(will, 1):
        try:
            update_desc(p['product_id'], p['new_desc'])
    ...
    log_path = LOG_DIR / f'pe1-push-{ts}.json'
