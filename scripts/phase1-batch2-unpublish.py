#!/usr/bin/env python3
"""
COLLECTIONS Phase 2 — Batch-2 (desk families) Source-Unpublish · PHASE 1/2 (publish toggles).
...
Reads the Phase 0 batch-2 classification JSON and acts on the TO_UNPUBLISH set only.
Any HALT-* row aborts the run.
...
Each LIVE write:
  ...
  3. hardened readback: REST published_at re-GET (must be null = authoritative)
     + throttled, cache-busted storefront curl (expect 301 -> keeper once edge clears)
...
STAMP = '2026-06-02'
p0 = json.load(open(os.path.join(ROOT, 'data', 'reports', f'phase0-batch2-classify-{STAMP}.json')))
...
print(f'MODE={MODE}  TO_UNPUBLISH={len(PLAN)}  (0 HALT)', file=sys.stderr)
...
for r in PLAN:
    ...
    with open(os.path.join(bdir, f'batch2-unpub-{handle}-{STAMP}.json'), 'w') as f:
    ...
    sf_note = 'OK 301->keeper' if sf_ok else ('edge-lag (REST authoritative)' if code == '200' else f'unexpected {sf}')
...
if args.live:
    ...
    lpath = os.path.join(ldir, f'batch2-unpublish-{STAMP}.json')
