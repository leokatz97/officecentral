...
Reads /tmp/b4s6_batch.json (the approved 27-handle manifest), queries each
product by handle via the Admin GraphQL API, captures the canonical fields, and
writes a pre-write snapshot to data/backups/session-6-{handle}-pre-{ts}.json
...
def main():
    manifest = json.load(open("/tmp/b4s6_batch.json"))
    ...
    for h in handles:
        ...
        path = os.path.join(bdir, f"session-6-{h}-pre-{ts}.json")
        ...
        time.sleep(0.25)
    json.dump({"ts": ts, "results": results}, open("/tmp/b4s6_state.json", "w"), indent=2)
