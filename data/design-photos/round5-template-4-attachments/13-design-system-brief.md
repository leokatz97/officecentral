...
**Then attach:**
1. **The new brand mark:** `data/logos/bbi-logo-hires.png` (this locks the anchor red and charcoal — the system must follow it, not the other way around)
2. **3–5 screenshots** of the current live brantbusinessinteriors.com (homepage, PDP, collection, header, footer)
...
- **Brand red is locked at `#D4252A`** — taken directly from the attached logo. Don't propose a different value. **Two reds required:**
  - `red-surface` = `#D4252A` exact, used on buttons, banners, badges (large surfaces).
  - `red-text` = ~12–18% darker variant (you propose, test on white). Must hit AA 4.5:1 on white.
- **Anchor neutral is locked at `#0B0B0C`** charcoal — taken directly from the logo wordmarks. Use this for headings and body text. NOT navy. (Earlier briefs proposed navy; the brand mark supersedes that.)
- **Red density rule:** red should appear on roughly 5–8% of any given screen. Primary CTAs, key badges, hover accents only. Body links should default to charcoal, not red. Reserve red for action.
